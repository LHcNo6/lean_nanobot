...
from step116.tools.cli_apps import (
    CliApp,
    CliAppManager,
    CliAppsTool,
...
class TestCliAppsTool:
    ...
    def _make_ctx(self, manager=None):
        ...
        from step116.tools.cron import _CronStore
...
class TestBuildCliAppManager:
    """build_cli_app_manager 配置 -> 管理器（step116）。"""
    ...
    def test_registers_from_schema_config(self) -> None:
        ...
        from step116.config.schema import CliAppSpec, CliAppsConfig
...
class TestCliAppsToolRealExecution:
    """CliAppsTool 端到端真实执行（step116，已接线 manager）。"""
