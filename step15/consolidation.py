...
from step15.helpers import (
    estimate_message_tokens,
    find_legal_message_start,
    truncate_text,
)
from step15.llm import Runtime
from step15.memory import MemoryStore, _ARCHIVE_SUMMARY_MAX_CHARS, _RAW_ARCHIVE_MAX_CHARS
...
def _consolidation_boundary(unconsolidated, target_tokens):
    kept_tokens = 0
    keep_count = 0
...
class Consolidator:
    ...
    async def maybe_consolidate(self, session, max_tokens, model=None):
        unconsolidated = session.messages[session.last_consolidated:]
        if not unconsolidated:
            return None
        from step15.helpers import estimate_prompt_tokens
    ...
    async def _archive_llm(self, messages, model=None):
    ...
    async def maybe_consolidate_by_tokens(self, session, *, runtime, replay_max_messages=None):
    ...
    async def compact_idle_session(self, session_key, *, runtime, max_suffix=8):
        ...
        async with lock:
            ...
            if not messages_to_summarize:
                return ""
            from step15.session import safe_filename
            start = max(0, len(messages_to_summarize) - max_suffix)
            kept = messages_to_summarize[start:]
            removed = messages_to_summarize[:start]
            summary = None
            if removed:
                summary = await self.archive(removed, runtime=runtime, session_key=session_key)
            if summary and summary != "(nothing)":
                session.metadata["_last_summary"] = {
                    "text": summary,
                    "last_active": session.updated_at,
                }
            session.messages = session.messages[:session.last_consolidated] + kept
