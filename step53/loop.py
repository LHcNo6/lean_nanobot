...
from step53.bus.outbound_events import (
    RetryWaitEvent,
    StreamEndEvent,
    StreamedResponseEvent,
    outbound_message_for_event,
)
from step53.bus.progress import build_bus_progress_callback
from step53.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step53.autocompact import AutoCompact
from step53.command import CommandContext, CommandRouter, register_builtin_commands
from step53.consolidation import Consolidator
from step53.context import ContextBuilder, RequestContext
...
from step53.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step53.memory import MemoryStore
from step53.pairing import PairingStore
from step53.runtime_context import (
...
from step53.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step53.security.workspace_access import WorkspaceScopeResolver
from step53.session import Session, SessionManager
from step53.session import turn_continuation
from step53.session.history_visibility import HIDDEN_HISTORY_META
from step53.session.keys import UNIFIED_SESSION_KEY
from step53.subagent import SubagentManager
from step53.tool import ToolRegistry
from step53.context import ToolContext
from step53.loader import ToolLoader
from step53.helpers import truncate_text
from step53.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step53 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step53：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
...
class AgentLoop:
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step53.providers.factory import make_provider
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step53：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step53：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step53：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step53.session.manager import replay_max_messages_for_context
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step53：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step53：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step53：将 runtime_context 元数据设置到历史消息
