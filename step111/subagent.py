...
from step111.bus.events import InboundMessage
from step111.config.schema import ToolsConfig
from step111.context import ToolContext
from step111.hook import AgentHook, AgentHookContext
...
class SubagentManager:
    ...
    step111 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step111：manager 自己持有的长命令会话管理器，跨子代理共享
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        scope 的工具（exec/文件/搜索/web/write_stdin/apply_patch 共 11 个）；
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        step111：工具集改为每次 spawn 时经 ``_build_tools()`` 新建 ——
        ...
        Args:
            ...
            task: 用户任务描述。
            label: 展示标签。
            origin: 回传路由信息 dict（channel/chat_id/session_key）。
            status: 本任务的实时状态对象。
        """
