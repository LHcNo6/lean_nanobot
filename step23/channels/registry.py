...
import step23.channels as channels_pkg
...
def load_channel_class(module_name: str) -> type["BaseChannel"]:
    ...
    mod = importlib.import_module(f"step23.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, BaseChannel) and obj is not BaseChannel:
            return obj
    raise ImportError(f"No BaseChannel subclass in step23.channels.{module_name}")
