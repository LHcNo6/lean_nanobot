...
from step23.channel import BaseChannel
from step23.channels.cli import CliChannel
from step23.command import CommandContext, CommandRouter, normalize_command_text
from step23.consolidation import Consolidator, _consolidation_boundary
from step23.goal_state import *
from step23.helpers import estimate_message_tokens, estimate_prompt_tokens
from step23.llm import Runtime
...
from step23.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step23.llm import LLMResponse, ToolCallRequest
from step23.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step23.manager import ChannelManager
from step23.pairing import PAIRING_CODE_META_KEY, PairingStore
from step23.provider import LLMProvider
from step23.governance import ContextGovernanceConfig, ContextGovernor
from step23.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step23.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step23.runner import AgentRunSpec, AgentRunner
from step23.session import Session, SessionManager
from step23.subagent import SubagentManager, SubagentStatus
from step23.tool import ToolRegistry, Tool
...
from step23.events import StreamDeltaEvent
...
from step23.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step23.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step23.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step23.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step23.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step23.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step23.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step23.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step23.context import reset_request_context
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step23.tool import tool_parameters
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step23.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step23.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step23.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step23.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step23.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step23.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step23.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step23.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step23.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
