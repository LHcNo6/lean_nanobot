...
from step54.autocompact import AutoCompact
from step54.bus import MessageBus
from step54.channel import BaseChannel
from step54.channels.cli import CliChannel
from step54.command import CommandContext, CommandRouter, normalize_command_text
from step54.consolidation import Consolidator, _consolidation_boundary
from step54.goal_state import *
from step54.helpers import estimate_message_tokens, estimate_prompt_tokens
from step54.llm import Runtime
from step54.memory import MemoryStore
from step54.context import ContextBuilder
from step54.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step54.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step54.llm import LLMResponse, ToolCallRequest
from step54.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step54.manager import ChannelManager
from step54.pairing import PAIRING_CODE_META_KEY, PairingStore
from step54.provider import LLMProvider
from step54.governance import ContextGovernanceConfig, ContextGovernor
from step54.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step54.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step54.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step54.session import Session, SessionManager
from step54.subagent import SubagentManager, SubagentStatus
from step54.tool import ToolRegistry, Tool
from step54.tools.long_task import CreateGoalTool, UpdateGoalTool
from step54.tools.echo import EchoTool
from step54.tools.spawn import SpawnTool
from step54.bus.events import StreamDeltaEvent
from step54.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step54.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step54.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step54.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step54.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step54.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step54.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step54.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step54.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step54.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step54.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step54：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step54：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step54.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step54.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step54.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step54.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step54.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step54.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step54.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step54.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step54.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step54.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step54.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step54.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step54：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step54.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step54.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep54StateTrace(unittest.IsolatedAsyncioTestCase):
    """step54：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step54.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step54.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep54SaveTurnEnhancements(unittest.TestCase):
    """step54：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step54.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep54CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step54：_state_command shortcut 持久化。"""
...
class Teststep54DropMalformedToolCalls(unittest.TestCase):
    """step54：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep54MalformedRetryMessages(unittest.TestCase):
    """step54：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep54MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step54：_request_malformed_fallback 降级无工具请求。"""
...
class Teststep54FinalizationRetryMessages(unittest.TestCase):
    """step54：_finalization_retry_messages 构造重试消息。"""
...
class Teststep54FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step54：空响应耗尽后发 finalization retry 集成测试。"""
...
class Teststep54ExtractReasoning(unittest.TestCase):
    """step54：extract_reasoning 剥离推理内容。"""

    def test_extract_reasoning_strips_think_tags(self):
        """<think> 标签从 content 中剥离，reasoning 被提取。"""
        from step54.helpers import extract_reasoning
...
class Teststep54ReasoningIntegration(unittest.IsolatedAsyncioTestCase):
    """step54：reasoning 提取 + emit_reasoning 集成测试。"""
...
class Teststep54FinalizeContent(unittest.IsolatedAsyncioTestCase):
    """step54：hook.finalize_content 被调用。"""
...
class Teststep54UsageTools(unittest.TestCase):
    """step54：usage 工具方法（_usage_dict / _usage_total / _merge_usage）。"""

    def test_usage_dict_converts_values(self):
        """_usage_dict 正确转换 usage 字典为 int，过滤非数字值。"""
        from step54.runner import AgentRunner
...
class Teststep54UsageEstimate(unittest.TestCase):
    """step54：_estimate_response_usage / _usage_or_estimate。"""
    ...
    def test_estimate_response_usage_returns_total_and_estimated(self):
        ...
        from step54.runner import AgentRunner, AgentRunSpec, LLMResponse
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hello"}],
            tools=_MockToolRegistry(),
            provider=_MockProvider(),
...
class Teststep54AccumulateUsage(unittest.TestCase):
    """step54：_accumulate_usage 累计所有键。"""

    def test_accumulate_usage_counts_all_keys(self):
        """_accumulate_usage 累计所有键（包括 total_tokens/estimated_tokens）。"""
        from step54.runner import AgentRunner, LLMResponse
...
class _step54CancelledRegistry:
...
class _step54TrackingHook(AgentHook):
    """记录工具执行 hook 调用。"""
    def __init__(self):
        self.before_calls = []
        self.after_calls = []
...
class Teststep54RunTool(unittest.IsolatedAsyncioTestCase):
    """step54：_run_tool 三元组返回 + hook 生命周期。"""

    def _make_ctx(self):
        from step54.hook import AgentHookContext
        return AgentHookContext(iteration=0, messages=[])

    def _make_spec(self, registry):
        from step54.runner import AgentRunSpec
        return AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
            tools=registry,
            provider=_MockProvider(),
    ...
    async def test_run_tool_returns_triple(self):
        ...
        from step54.runner import AgentRunner
        from step54.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step54EchoRegistry()
        spec = self._make_spec(registry)
        hook = _step54TrackingHook()
        ctx = self._make_ctx()
    ...
    async def test_run_tool_calls_on_error_hook(self):
        ...
        self.assertIsNone(error)  # step54 不设 fatal_error（留到 step54 fail_on_tool_error）
    ...
    async def test_run_tool_cancelled_error_propagates(self):
        ...
        from step54.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step54CancelledRegistry()
        spec = self._make_spec(registry)
        hook = _step54TrackingHook()
        ctx = self._make_ctx()
        with self.assertRaises(asyncio.CancelledError):
...
class Teststep54ExecuteToolBatch(unittest.IsolatedAsyncioTestCase):
    """step54：_execute_tool_batch 三元组返回。"""

    async def test_execute_tool_batch_returns_triple(self):
        """_execute_tool_batch 返回 (results, events, fatal_error)。"""
        from step54.runner import AgentRunner, AgentRunSpec
        from step54.governance import ContextGovernanceConfig
        from step54.hook import AgentHookContext, AgentHook
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step54EchoRegistry("ok")
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
...
class Teststep54RunCancelledError(unittest.IsolatedAsyncioTestCase):
    """step54：run() 中 CancelledError 分离。"""

    async def test_run_cancelled_error_no_on_error(self):
        """run() 中 CancelledError 不调 on_error。"""
        from step54.runner import AgentRunner, AgentRunSpec
        from step54.hook import AgentHook
...
class Teststep54SSRFDetection(unittest.TestCase):
    """step54：SSRF 检测方法。"""
...
class Teststep54ClassifyViolation(unittest.TestCase):
    """step54：_classify_violation 统一分类。"""
...
class Teststep54RepeatedLookup(unittest.TestCase):
    """step54：重复外部查找和 workspace 违规检测。"""

    def test_repeated_external_lookup_blocks_after_2(self):
        """重复外部查找 2 次后阻断。"""
        from step54.helpers import repeated_external_lookup_error
        counts = {}
        # 前 2 次不阻断
    ...
    def test_repeated_workspace_violation_escalates_after_2(self):
        ...
        from step54.helpers import repeated_workspace_violation_error
        counts = {}
        # 前 2 次不升级
...
class Teststep54RunToolSecurity(unittest.IsolatedAsyncioTestCase):
    """step54：_run_tool 中安全检测集成。"""
...
class TestStep52SpecFields(unittest.TestCase):
    ...
    def test_tool_events_default_empty(self):
        ...
        from step54.runner import AgentRunResult
        result = AgentRunResult(final_content="hi", messages=[])
        self.assertEqual(result.tool_events, [])


class Teststep54FailOnToolError(unittest.IsolatedAsyncioTestCase):
    """step54：fail_on_tool_error 终止 turn。"""
...
class Teststep54ToolEvents(unittest.IsolatedAsyncioTestCase):
    """step54：tool_events 收集。"""
...
class Teststep54SpecFields(unittest.TestCase):
    """step54：AgentRunSpec 新字段。"""
...
class Teststep54IncrementalThinkExtractor(unittest.IsolatedAsyncioTestCase):
    """step54：IncrementalThinkExtractor 基本功能。"""

    async def test_feed_extracts_think(self):
        """feed 提取 <think> 块中的新文本并 emit。"""
        from step54.helpers import IncrementalThinkExtractor
        emitted = []
        async def emit(text):
            emitted.append(text)
...
class Teststep54StreamingThinking(unittest.IsolatedAsyncioTestCase):
    """step54：流式 thinking 提取。"""
...
class TestStep54StreamSegmentation(unittest.IsolatedAsyncioTestCase):
    ...
    def test_stream_delta_typed_event_has_stream_id(self):
        ...
        from step54.bus.outbound_events import StreamDeltaEvent as TypedDelta
