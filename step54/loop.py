...
from step54.bus.outbound_events import (
    RetryWaitEvent,
...
from step54.bus.progress import build_bus_progress_callback
from step54.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step54.autocompact import AutoCompact
from step54.command import CommandContext, CommandRouter, register_builtin_commands
from step54.consolidation import Consolidator
from step54.context import ContextBuilder, RequestContext
...
from step54.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step54.memory import MemoryStore
from step54.pairing import PairingStore
from step54.runtime_context import (
...
from step54.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step54.security.workspace_access import WorkspaceScopeResolver
from step54.session import Session, SessionManager
from step54.session import turn_continuation
from step54.session.history_visibility import HIDDEN_HISTORY_META
from step54.session.keys import UNIFIED_SESSION_KEY
from step54.subagent import SubagentManager
from step54.tool import ToolRegistry
from step54.context import ToolContext
from step54.loader import ToolLoader
from step54.helpers import truncate_text
from step54.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step54 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step54：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step54：跟踪后台任务，shutdown 时 drain
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step54.providers.factory import make_provider
    ...
    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step54：登记+完成自动移除）。
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step54：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step54：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step54：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step54：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step54.session.manager import replay_max_messages_for_context
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        # step54：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step54：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step54：typed event 路径，带 stream_id
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step54：带 stream_id，resuming=False 时递增 segment。
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step54：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step54：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step54：将 runtime_context 元数据设置到历史消息
