...
from step52.autocompact import AutoCompact
from step52.bus import MessageBus
from step52.channel import BaseChannel
from step52.channels.cli import CliChannel
from step52.command import CommandContext, CommandRouter, normalize_command_text
from step52.consolidation import Consolidator, _consolidation_boundary
from step52.goal_state import *
from step52.helpers import estimate_message_tokens, estimate_prompt_tokens
from step52.llm import Runtime
from step52.memory import MemoryStore
from step52.context import ContextBuilder
from step52.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step52.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step52.llm import LLMResponse, ToolCallRequest
from step52.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step52.manager import ChannelManager
from step52.pairing import PAIRING_CODE_META_KEY, PairingStore
from step52.provider import LLMProvider
from step52.governance import ContextGovernanceConfig, ContextGovernor
from step52.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step52.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step52.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step52.session import Session, SessionManager
from step52.subagent import SubagentManager, SubagentStatus
from step52.tool import ToolRegistry, Tool
from step52.tools.long_task import CreateGoalTool, UpdateGoalTool
from step52.tools.echo import EchoTool
from step52.tools.spawn import SpawnTool
from step52.bus.events import StreamDeltaEvent
from step52.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step52.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step52.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step52.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step52.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step52.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step52.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step52.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step52.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step52.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step52.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step52：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step52：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step52.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step52.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step52.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step52.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step52.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step52.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step52.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step52.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step52.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step52.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step52.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step52.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step52.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step52：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step52.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step52.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep52StateTrace(unittest.IsolatedAsyncioTestCase):
    """step52：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step52.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step52.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep52SaveTurnEnhancements(unittest.TestCase):
    """step52：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step52.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep52CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step52：_state_command shortcut 持久化。"""
...
class Teststep52DropMalformedToolCalls(unittest.TestCase):
    """step52：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep52MalformedRetryMessages(unittest.TestCase):
    """step52：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep52MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step52：_request_malformed_fallback 降级无工具请求。"""
...
class Teststep52FinalizationRetryMessages(unittest.TestCase):
    """step52：_finalization_retry_messages 构造重试消息。"""
...
class Teststep52FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step52：空响应耗尽后发 finalization retry 集成测试。"""
...
class Teststep52ExtractReasoning(unittest.TestCase):
    """step52：extract_reasoning 剥离推理内容。"""

    def test_extract_reasoning_strips_think_tags(self):
        """<think> 标签从 content 中剥离，reasoning 被提取。"""
        from step52.helpers import extract_reasoning
...
class Teststep52ReasoningIntegration(unittest.IsolatedAsyncioTestCase):
    """step52：reasoning 提取 + emit_reasoning 集成测试。"""
...
class Teststep52FinalizeContent(unittest.IsolatedAsyncioTestCase):
    """step52：hook.finalize_content 被调用。"""
...
class Teststep52UsageTools(unittest.TestCase):
    """step52：usage 工具方法（_usage_dict / _usage_total / _merge_usage）。"""

    def test_usage_dict_converts_values(self):
        """_usage_dict 正确转换 usage 字典为 int，过滤非数字值。"""
        from step52.runner import AgentRunner
...
class Teststep52UsageEstimate(unittest.TestCase):
    """step52：_estimate_response_usage / _usage_or_estimate。"""
    ...
    def test_estimate_response_usage_returns_total_and_estimated(self):
        ...
        from step52.runner import AgentRunner, AgentRunSpec, LLMResponse
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hello"}],
            tools=_MockToolRegistry(),
            provider=_MockProvider(),
...
class Teststep52AccumulateUsage(unittest.TestCase):
    """step52：_accumulate_usage 累计所有键。"""

    def test_accumulate_usage_counts_all_keys(self):
        """_accumulate_usage 累计所有键（包括 total_tokens/estimated_tokens）。"""
        from step52.runner import AgentRunner, LLMResponse
...
class _step52CancelledRegistry:
...
class _step52TrackingHook(AgentHook):
    """记录工具执行 hook 调用。"""
    def __init__(self):
        self.before_calls = []
        self.after_calls = []
...
class Teststep52RunTool(unittest.IsolatedAsyncioTestCase):
    """step52：_run_tool 三元组返回 + hook 生命周期。"""

    def _make_ctx(self):
        from step52.hook import AgentHookContext
        return AgentHookContext(iteration=0, messages=[])

    def _make_spec(self, registry):
        from step52.runner import AgentRunSpec
        return AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
            tools=registry,
            provider=_MockProvider(),
    ...
    async def test_run_tool_returns_triple(self):
        ...
        from step52.runner import AgentRunner
        from step52.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step52EchoRegistry()
        spec = self._make_spec(registry)
        hook = _step52TrackingHook()
        ctx = self._make_ctx()
    ...
    async def test_run_tool_calls_on_error_hook(self):
        ...
        self.assertIsNone(error)  # step52 不设 fatal_error（留到 step52 fail_on_tool_error）
    ...
    async def test_run_tool_cancelled_error_propagates(self):
        ...
        from step52.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step52CancelledRegistry()
        spec = self._make_spec(registry)
        hook = _step52TrackingHook()
        ctx = self._make_ctx()
        with self.assertRaises(asyncio.CancelledError):
...
class Teststep52ExecuteToolBatch(unittest.IsolatedAsyncioTestCase):
    """step52：_execute_tool_batch 三元组返回。"""

    async def test_execute_tool_batch_returns_triple(self):
        """_execute_tool_batch 返回 (results, events, fatal_error)。"""
        from step52.runner import AgentRunner, AgentRunSpec
        from step52.governance import ContextGovernanceConfig
        from step52.hook import AgentHookContext, AgentHook
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step52EchoRegistry("ok")
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
...
class Teststep52RunCancelledError(unittest.IsolatedAsyncioTestCase):
    """step52：run() 中 CancelledError 分离。"""

    async def test_run_cancelled_error_no_on_error(self):
        """run() 中 CancelledError 不调 on_error。"""
        from step52.runner import AgentRunner, AgentRunSpec
        from step52.hook import AgentHook
...
class Teststep52SSRFDetection(unittest.TestCase):
    """step52：SSRF 检测方法。"""
...
class Teststep52ClassifyViolation(unittest.TestCase):
    """step52：_classify_violation 统一分类。"""
...
class Teststep52RepeatedLookup(unittest.TestCase):
    """step52：重复外部查找和 workspace 违规检测。"""

    def test_repeated_external_lookup_blocks_after_2(self):
        """重复外部查找 2 次后阻断。"""
        from step52.helpers import repeated_external_lookup_error
        counts = {}
        # 前 2 次不阻断
    ...
    def test_repeated_workspace_violation_escalates_after_2(self):
        ...
        from step52.helpers import repeated_workspace_violation_error
        counts = {}
        # 前 2 次不升级
...
class Teststep52RunToolSecurity(unittest.IsolatedAsyncioTestCase):
    """step52：_run_tool 中安全检测集成。"""
...
class TestStep52SpecFields(unittest.TestCase):
    ...
    def test_tool_events_default_empty(self):
        ...
        from step52.runner import AgentRunResult
        result = AgentRunResult(final_content="hi", messages=[])
        self.assertEqual(result.tool_events, [])


class TestStep52FailOnToolError(unittest.IsolatedAsyncioTestCase):
    """step52：fail_on_tool_error 终止 turn。"""
...
class TestStep52ToolEvents(unittest.IsolatedAsyncioTestCase):
    """step52：tool_events 收集。"""
