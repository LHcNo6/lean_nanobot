...
from step35.bus.outbound_events import (
    RetryWaitEvent,
    StreamEndEvent,
    StreamedResponseEvent,
    outbound_message_for_event,
)
from step35.bus.progress import build_bus_progress_callback
from step35.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step35.autocompact import AutoCompact
from step35.command import CommandContext, CommandRouter, register_builtin_commands
from step35.consolidation import Consolidator
from step35.context import ContextBuilder, RequestContext
from step35.goal_state import goal_state_runtime_lines, sustained_goal_active
from step35.hook import (
    AgentHook,
    AgentHookContext,
    AgentTurnHookSpec,
    build_agent_turn_hook,
)
from step35.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step35.memory import MemoryStore
from step35.pairing import PairingStore
from step35.runtime_context import (
    RUNTIME_CONTEXT_HISTORY_META,
    RuntimeContextBlock,
    append_runtime_context,
    resolve_runtime_context,
)
from step35.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step35.security.workspace_access import WorkspaceScopeResolver
from step35.session import Session, SessionManager
from step35.session import turn_continuation
from step35.session.history_visibility import HIDDEN_HISTORY_META
from step35.session.keys import UNIFIED_SESSION_KEY
from step35.subagent import SubagentManager
from step35.tool import ToolRegistry
from step35.context import ToolContext
from step35.loader import ToolLoader
from step35.helpers import truncate_text
from step35.utils.cancellation import task_is_cancelling
...
class AgentLoop:
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step35.providers.factory import make_provider
    ...
    def _build_initial_messages(
        ...
        include_memory_recent_history / session_key / unified_session 等参数
        （需要媒体处理和统一会话基础设施，留待后续 step）。
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step35.session.manager import replay_max_messages_for_context
    ...
    async def _run_agent_loop(
        ...
        *,
        ...
        暂不包含：``_sync_subagent_runtime_limits``（需要 self.max_iterations 属性）、
        contextvar 绑定（runner 已内部绑定）、``llm_timeout_s``（需要持续目标
        基础设施完善）、``turn_scopes`` / ``hook_factories`` / ``ephemeral``
        （需要对应基础设施）。
        ...
        if spec.hook is not None and spec.hook.wants_streaming():
            effective_stream = spec.hook  # type: ignore[assignment]
            effective_stream_end = spec.hook  # type: ignore[assignment]
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        from step35.runner import AgentRunResult
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # step27: 流式已交付时在最终消息上挂 StreamedResponseEvent，让通道
        # 知道内容已经以 delta 形式流过（对齐 nanobot ``_state_respond``）。
        event = None
        if ctx.on_stream is not None and ctx.result.stop_reason not in {
            "error",
            "tool_error",
        }:
