...
`normalize_command_text`、`CommandContext`。priority 档保留 API 但 step21
暂无消费方（无 /stop 类命令），exact/prefix 由 loop 的 COMMAND 状态使用。
"""
...
if TYPE_CHECKING:
    from step21.events import InboundMessage, OutboundMessage
    from step21.session import Session
