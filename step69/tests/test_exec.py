...
from step69.tool import ToolRegistry, ToolResult
from step69.tools.shell import ExecTool, _DEFAULT_DENY_PATTERNS
...
class TestExecDangerous:
    ...
    def test_rm_rf_blocked(self, tmp_path: Path) -> None:
        ...
        assert isinstance(result, ToolResult)
        assert result.is_error
        assert "blocked for safety" in str(result).lower()
    ...
    def test_rm_without_rf_not_blocked(self, tmp_path: Path) -> None:
        ...
        # rm 不带 -rf 应该通过黑名单检查
        danger = tool._check_dangerous("rm file.txt")
