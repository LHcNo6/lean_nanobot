...
from step121.config.schema import Config, ToolsConfig
...
from step121.security.workspace_access import default_workspace_scope
from step121.subagent import SubagentManager, _flatten_tools_config
...
class TestBuildToolsWhitelist(unittest.IsolatedAsyncioTestCase):
    ...
    def test_build_tools_contains_exactly_subagent_set(self) -> None:
        """契约 B1：恰含 13 个 subagent-scope 工具，不多不少。"""
...
class TestRecursiveSpawnPrevention(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawned_subagent_cannot_recursive_spawn(self) -> None:
        """契约 B3：子代理工具集不含 spawn → 递归 spawn 永不被真正启动。

        step121 起 ``fail_on_tool_error`` 随配置生效（默认 True），子代理在
        收到 "spawn not found" 错误后可能直接终止（而非发第二轮请求）。无论
        该开关如何，关键安全不变量是：嵌套 spawn 请求绝不会创建新的子代理
        任务。此处只断言结构性不变量，不被调用次数等实现细节绑架。
        """
        bus = MessageBus()
        ...
        # 2) 确定性证明：子代理注册表根本不含 spawn 工具（结构性防递归），
        #    即便模型请求 spawn，工具层也不存在，嵌套任务无从启动。
        self.assertNotIn("spawn", manager._build_tools()._tools)
        # 3) 子代理确实运行过（至少调用了一次模型），证明上述隔离在真实运行期生效。
        self.assertTrue(provider.calls, "subagent never invoked model")
...
class TestSubagentRequestContextBinding(unittest.IsolatedAsyncioTestCase):
    """step121：子代理运行期 RequestContext / workspace_scope 绑定（契约 B1-B3）。"""

    def setUp(self) -> None:
        """每个用例新建 manager，并置非空 provider 以越过 _run_subagent 的早退守卫。"""
        self.manager = SubagentManager(bus=MessageBus(), config=Config(), workspace=".")
        # provider 非 None 才能让 _run_subagent 真正跑 runner（此处 runner 被替换）。
        self.manager._provider = object()

    async def _run_with_fake_runner(self, origin: dict | None) -> Any:
        ...
        self.manager.runner.run = fake_run
        await self.manager.spawn(task="t", origin=origin)
        for _ in range(200):
            if self.manager.get_running_count() == 0:
                break
            await asyncio.sleep(0.01)
        return captured["spec"]

    async def test_spawn_binds_origin_context_to_spec(self) -> None:
        """契约 B2：origin 透传为 spec.request_context / spec.workspace_scope。"""
        ws = default_workspace_scope("C:/ws", True)
        origin = {
            "channel": "cli",
            "chat_id": "c1",
            "session_key": "s1",
        ...
        spec = await self._run_with_fake_runner(origin)
        self.assertIsInstance(spec.request_context, RequestContext)
        self.assertEqual(spec.request_context.session_key, "s1")
        self.assertEqual(spec.request_context.channel, "cli")
        self.assertEqual(spec.request_context.chat_id, "c1")
        self.assertEqual(spec.request_context.message_id, "m1")
        self.assertIs(spec.workspace_scope, ws)

    async def test_default_origin_falls_back(self) -> None:
        """契约 B3：缺省 origin 时 session_key / workspace_scope 为 None（无回归）。"""
        spec = await self._run_with_fake_runner(None)
        self.assertIsNone(spec.request_context.session_key)
        self.assertIsNone(spec.workspace_scope)
        # 旧式关键字形参仍向后兼容
        captured: dict[str, Any] = {}
        ...
        self.manager.runner.run = fake_run
        await self.manager.spawn(task="t2", session_key="legacy")
        for _ in range(200):
            if self.manager.get_running_count() == 0:
                break
            await asyncio.sleep(0.01)
        self.assertEqual(captured["spec"].request_context.session_key, "legacy")


class TestSubagentCliAppManagerWiring(unittest.IsolatedAsyncioTestCase):
    """step121：子代理 _build_tools 注入 cli_app_manager。"""

    def test_build_tools_wires_registered_apps(self) -> None:
        """Config.cli_apps.apps 注册的应用应出现在子代理 run_cli_app 管理器里。"""
        from step121.config.schema import CliAppSpec

        config = Config()
        config.cli_apps.apps = [CliAppSpec(name="lint", command="ruff", description="Lint")]
        manager = SubagentManager(bus=MessageBus(), config=config, workspace=".")
        registry = manager._build_tools()
        tool = registry._tools.get("run_cli_app")
        self.assertIsNotNone(tool)
        self.assertIn("lint", tool._manager.list_names())
        self.assertEqual(tool._manager.get("lint").command, "ruff")

    def test_build_tools_empty_without_apps(self) -> None:
        """无 cli_apps 配置时子代理 run_cli_app 管理器为空（向后兼容）。"""
        manager = SubagentManager(bus=MessageBus(), config=Config(), workspace=".")
        registry = manager._build_tools()
        tool = registry._tools.get("run_cli_app")
        self.assertIsNotNone(tool)
        self.assertEqual(tool._manager.list_names(), [])


class TestSubagentRuntimeLimitSync(unittest.IsolatedAsyncioTestCase):
    """step121：子代理 llm_timeout_s 同步父会话墙钟超时策略（契约 D1+D2）。"""

    def _make_manager(self, callback):
        """构造注入了回调的子代理管理器，并置非空 provider 越过早退守卫。"""
        mgr = SubagentManager(
            bus=MessageBus(),
            config=Config(),
            workspace=".",
            llm_wall_timeout_for_session=callback,
        )
        mgr._provider = object()
        return mgr

    async def _run_with_fake_runner(self, mgr, origin) -> Any:
        """用记录 spec 的假 runner 替换 manager.runner.run，等待后台任务结束。"""
        captured: dict[str, Any] = {}
    ...
    async def test_sustained_goal_session_disables_timeout(self) -> None:
        """父会话为 sustained-goal（回调返回 0.0）时子代理 llm_timeout_s == 0.0。"""
        cb = lambda sk: 0.0 if sk == "goal-session" else None
        mgr = self._make_manager(cb)
        origin = {
            "channel": "cli", "chat_id": "c", "session_key": "goal-session",
            "message_id": "m", "workspace_scope": None,
        }
        spec = await self._run_with_fake_runner(mgr, origin)
        self.assertEqual(spec.llm_timeout_s, 0.0)

    async def test_normal_session_uses_none(self) -> None:
        """普通父会话（回调返回 None）时子代理 llm_timeout_s is None。"""
        cb = lambda sk: 0.0 if sk == "goal-session" else None
        mgr = self._make_manager(cb)
        origin = {
            "channel": "cli", "chat_id": "c", "session_key": "normal",
            "message_id": "m", "workspace_scope": None,
        }
        spec = await self._run_with_fake_runner(mgr, origin)
        self.assertIsNone(spec.llm_timeout_s)

    async def test_no_callback_defaults_none(self) -> None:
        """未注入回调时子代理 llm_timeout_s is None（与 step116 行为一致，无回归）。"""
        mgr = self._make_manager(None)
        origin = {
            "channel": "cli", "chat_id": "c", "session_key": "x",
            "message_id": "m", "workspace_scope": None,
        }
        spec = await self._run_with_fake_runner(mgr, origin)
        self.assertIsNone(spec.llm_timeout_s)
