...
from step57.channel import BaseChannel
from step57.channels.cli import CliChannel
from step57.command import CommandContext, CommandRouter, normalize_command_text
from step57.consolidation import Consolidator, _consolidation_boundary
from step57.goal_state import *
from step57.helpers import estimate_message_tokens, estimate_prompt_tokens
from step57.llm import Runtime
from step57.memory import MemoryStore
from step57.context import ContextBuilder
from step57.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step57.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step57.llm import LLMResponse, ToolCallRequest
from step57.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step57.manager import ChannelManager
from step57.pairing import PAIRING_CODE_META_KEY, PairingStore
from step57.provider import LLMProvider
from step57.governance import ContextGovernanceConfig, ContextGovernor
from step57.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step57.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step57.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step57.session import Session, SessionManager
from step57.subagent import SubagentManager, SubagentStatus
from step57.tool import ToolRegistry, Tool
from step57.tools.long_task import CreateGoalTool, UpdateGoalTool
from step57.tools.echo import EchoTool
from step57.tools.spawn import SpawnTool
from step57.bus.events import StreamDeltaEvent
from step57.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step57.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step57.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step57.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step57.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step57.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step57.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step57.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step57.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step57.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step57.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step57：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step57：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step57.tool import tool_parameters
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step57.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step57.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step57.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step57.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step57.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step57.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step57.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step57.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step57.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step57.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step57：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step57.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class Teststep57StateTrace(unittest.IsolatedAsyncioTestCase):
    """step57：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step57.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step57.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep57SaveTurnEnhancements(unittest.TestCase):
    """step57：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step57.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep57CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step57：_state_command shortcut 持久化。"""
...
class Teststep57DropMalformedToolCalls(unittest.TestCase):
    """step57：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep57MalformedRetryMessages(unittest.TestCase):
    """step57：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep57MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step57：_request_malformed_fallback 降级无工具请求。"""
...
class Teststep57FinalizationRetryMessages(unittest.TestCase):
    """step57：_finalization_retry_messages 构造重试消息。"""
...
class Teststep57FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step57：空响应耗尽后发 finalization retry 集成测试。"""
...
class Teststep57ExtractReasoning(unittest.TestCase):
    """step57：extract_reasoning 剥离推理内容。"""

    def test_extract_reasoning_strips_think_tags(self):
        """<think> 标签从 content 中剥离，reasoning 被提取。"""
        from step57.helpers import extract_reasoning
...
class Teststep57ReasoningIntegration(unittest.IsolatedAsyncioTestCase):
    """step57：reasoning 提取 + emit_reasoning 集成测试。"""
...
class Teststep57FinalizeContent(unittest.IsolatedAsyncioTestCase):
    """step57：hook.finalize_content 被调用。"""
...
class Teststep57UsageTools(unittest.TestCase):
    """step57：usage 工具方法（_usage_dict / _usage_total / _merge_usage）。"""

    def test_usage_dict_converts_values(self):
        """_usage_dict 正确转换 usage 字典为 int，过滤非数字值。"""
        from step57.runner import AgentRunner
...
class Teststep57UsageEstimate(unittest.TestCase):
    """step57：_estimate_response_usage / _usage_or_estimate。"""
    ...
    def test_estimate_response_usage_returns_total_and_estimated(self):
        ...
        from step57.runner import AgentRunner, AgentRunSpec, LLMResponse
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hello"}],
            tools=_MockToolRegistry(),
            provider=_MockProvider(),
...
class Teststep57AccumulateUsage(unittest.TestCase):
    """step57：_accumulate_usage 累计所有键。"""

    def test_accumulate_usage_counts_all_keys(self):
        """_accumulate_usage 累计所有键（包括 total_tokens/estimated_tokens）。"""
        from step57.runner import AgentRunner, LLMResponse
...
class _step57CancelledRegistry:
...
class _step57TrackingHook(AgentHook):
    """记录工具执行 hook 调用。"""
    def __init__(self):
        self.before_calls = []
        self.after_calls = []
...
class Teststep57RunTool(unittest.IsolatedAsyncioTestCase):
    """step57：_run_tool 三元组返回 + hook 生命周期。"""

    def _make_ctx(self):
        from step57.hook import AgentHookContext
        return AgentHookContext(iteration=0, messages=[])

    def _make_spec(self, registry):
        from step57.runner import AgentRunSpec
        return AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
            tools=registry,
            provider=_MockProvider(),
    ...
    async def test_run_tool_returns_triple(self):
        ...
        from step57.runner import AgentRunner
        from step57.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step57EchoRegistry()
        spec = self._make_spec(registry)
        hook = _step57TrackingHook()
        ctx = self._make_ctx()
    ...
    async def test_run_tool_calls_on_error_hook(self):
        ...
        self.assertIsNone(error)  # step57 不设 fatal_error（留到 step57 fail_on_tool_error）
    ...
    async def test_run_tool_cancelled_error_propagates(self):
        ...
        from step57.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step57CancelledRegistry()
        spec = self._make_spec(registry)
        hook = _step57TrackingHook()
        ctx = self._make_ctx()
        with self.assertRaises(asyncio.CancelledError):
...
class Teststep57ExecuteToolBatch(unittest.IsolatedAsyncioTestCase):
    """step57：_execute_tool_batch 三元组返回。"""

    async def test_execute_tool_batch_returns_triple(self):
        """_execute_tool_batch 返回 (results, events, fatal_error)。"""
        from step57.runner import AgentRunner, AgentRunSpec
        from step57.governance import ContextGovernanceConfig
        from step57.hook import AgentHookContext, AgentHook
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step57EchoRegistry("ok")
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
...
class Teststep57RunCancelledError(unittest.IsolatedAsyncioTestCase):
    """step57：run() 中 CancelledError 分离。"""

    async def test_run_cancelled_error_no_on_error(self):
        """run() 中 CancelledError 不调 on_error。"""
        from step57.runner import AgentRunner, AgentRunSpec
        from step57.hook import AgentHook
...
class Teststep57SSRFDetection(unittest.TestCase):
    """step57：SSRF 检测方法。"""
...
class Teststep57ClassifyViolation(unittest.TestCase):
    """step57：_classify_violation 统一分类。"""
...
class Teststep57RepeatedLookup(unittest.TestCase):
    """step57：重复外部查找和 workspace 违规检测。"""

    def test_repeated_external_lookup_blocks_after_2(self):
        """重复外部查找 2 次后阻断。"""
        from step57.helpers import repeated_external_lookup_error
        counts = {}
        # 前 2 次不阻断
    ...
    def test_repeated_workspace_violation_escalates_after_2(self):
        ...
        from step57.helpers import repeated_workspace_violation_error
        counts = {}
        # 前 2 次不升级
...
class Teststep57RunToolSecurity(unittest.IsolatedAsyncioTestCase):
    """step57：_run_tool 中安全检测集成。"""
...
class TestStep52SpecFields(unittest.TestCase):
    ...
    def test_tool_events_default_empty(self):
        ...
        from step57.runner import AgentRunResult
        result = AgentRunResult(final_content="hi", messages=[])
        self.assertEqual(result.tool_events, [])


class Teststep57FailOnToolError(unittest.IsolatedAsyncioTestCase):
    """step57：fail_on_tool_error 终止 turn。"""
...
class Teststep57ToolEvents(unittest.IsolatedAsyncioTestCase):
    """step57：tool_events 收集。"""
...
class Teststep57SpecFields(unittest.TestCase):
    """step57：AgentRunSpec 新字段。"""
...
class Teststep57IncrementalThinkExtractor(unittest.IsolatedAsyncioTestCase):
    """step57：IncrementalThinkExtractor 基本功能。"""

    async def test_feed_extracts_think(self):
        """feed 提取 <think> 块中的新文本并 emit。"""
        from step57.helpers import IncrementalThinkExtractor
        emitted = []
        async def emit(text):
            emitted.append(text)
...
class Teststep57StreamingThinking(unittest.IsolatedAsyncioTestCase):
    """step57：流式 thinking 提取。"""
...
class TestStep54StreamSegmentation(unittest.IsolatedAsyncioTestCase):
    ...
    def test_stream_delta_typed_event_has_stream_id(self):
        ...
        from step57.bus.outbound_events import StreamDeltaEvent as TypedDelta
...
class TestStep55ModelRuntimeResolver(unittest.TestCase):
    """step57：ModelRuntimeResolver 核心功能。"""

    def _make_runtime(self, model="gpt-4", preset=None, sig=None):
        from step57.llm import GenerationSettings, LLMRuntime
        return LLMRuntime(
            provider=object(),
            model=model,
...
class TestStep55AgentLoopRuntime(unittest.TestCase):
    """step57：AgentLoop runtime 委托给 ModelRuntimeResolver。"""
...
class TestStep56DocumentUtils(unittest.TestCase):
    """step57：utils/document.py 附件处理。"""

    def test_is_image_file_png(self):
        from step57.utils.document import is_image_file
...
class TestStep56ImagePlaceholder(unittest.TestCase):
    """step57：image_placeholder_text。"""

    def test_with_path(self):
        from step57.helpers import image_placeholder_text
...
class TestStep56PrepareMessageMedia(unittest.TestCase):
    """step57：AgentLoop._prepare_message_media。"""
...
class TestStep57TurnContextFields(unittest.TestCase):
    """step57：TurnContext 移除 result/error/summary，改用扁平字段。"""

    def test_turn_context_no_result_field(self):
        """TurnContext 不再有 result 字段。"""
        from step57.loop import TurnContext
        ctx = TurnContext(
            msg=InboundMessage(content="hi"),
            session_key="test",
        )
