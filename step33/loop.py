...
from step33.bus.outbound_events import (
    RetryWaitEvent,
    StreamEndEvent,
    StreamedResponseEvent,
    outbound_message_for_event,
)
from step33.bus.progress import build_bus_progress_callback
from step33.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step33.autocompact import AutoCompact
from step33.command import CommandContext, CommandRouter, register_builtin_commands
from step33.consolidation import Consolidator
from step33.context import ContextBuilder, RequestContext
from step33.goal_state import goal_state_runtime_lines, sustained_goal_active
from step33.hook import (
    AgentHook,
    AgentHookContext,
    AgentTurnHookSpec,
    build_agent_turn_hook,
)
from step33.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step33.memory import MemoryStore
from step33.pairing import PairingStore
from step33.runtime_context import RuntimeContextBlock, resolve_runtime_context
from step33.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step33.security.workspace_access import WorkspaceScopeResolver
from step33.session import Session, SessionManager
from step33.session import turn_continuation
from step33.session.history_visibility import HIDDEN_HISTORY_META
from step33.session.keys import UNIFIED_SESSION_KEY
from step33.subagent import SubagentManager
from step33.tool import ToolRegistry
from step33.context import ToolContext
from step33.loader import ToolLoader
from step33.helpers import truncate_text
from step33.utils.cancellation import task_is_cancelling
...
class AgentLoop:
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step33.providers.factory import make_provider
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step33.session.manager import replay_max_messages_for_context
        ...
        # initial_messages 附加到当前用户消息；历史保持原始文本（不持久化）。
        ...
        # step29：续跑消息/_skip_user_persist 不持久化为用户输入（否则
        # "Continue the active sustained goal..." 合成文本会污染 LLM 历史）。
        ctx.user_persisted_early = False
        if turn_continuation.should_persist_user_message(ctx.msg.metadata):
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        if spec.hook is not None and spec.hook.wants_streaming():
            ctx.on_stream = spec.hook

        # step29：续跑字段落盘到 ctx（_state_save 用）＋策略决策排班续跑片。
        ctx.final_content = ctx.result.final_content
        ctx.all_messages = ctx.result.messages
        ctx.stop_reason = ctx.result.stop_reason
        # step32：错误文案和注入标记落盘到 ctx。
        ctx.error = ctx.result.error
        ctx.had_injections = ctx.result.had_injections
