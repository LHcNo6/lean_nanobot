...
from step62.bus.outbound_events import (
    RetryWaitEvent,
...
from step62.bus.progress import build_bus_progress_callback
from step62.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step62.autocompact import AutoCompact
from step62.command import CommandContext, CommandRouter, register_builtin_commands
from step62.consolidation import Consolidator
from step62.context import ContextBuilder, RequestContext
...
from step62.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step62.model_runtime import ModelRuntimeResolver
from step62.memory import MemoryStore
from step62.pairing import PairingStore
from step62.runtime_context import (
...
from step62.session import turn_continuation
from step62.session.history_visibility import HIDDEN_HISTORY_META
from step62.session.keys import UNIFIED_SESSION_KEY
from step62.subagent import SubagentManager
from step62.tool import ToolRegistry
from step62.context import ToolContext
from step62.loader import ToolLoader
from step62.helpers import image_placeholder_text, truncate_text
from step62.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step62 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step62：summary 改名为 pending_summary（对齐 nanobot）
    ...
    stop_reason: str = ""
    # step62：移除 error 字段（错误通过 final_content + stop_reason 传递）
    had_injections: bool = False
    # step62：usage 字典（tokens 信息）
    ...
    # step62：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
    trace: list[StateTraceEntry] = field(default_factory=list)
    # step62：补齐 turn 级配置字段（对齐 nanobot）。
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step62：跟踪后台任务，shutdown 时 drain
        ...
        # step62：通道/工具配置（对齐 nanobot，供 media 处理等消费）
        ...
        # step62：runtime 管理委托给 ModelRuntimeResolver
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step62.providers.factory import make_provider
        ...
        return cls(
            ...
            # step62：传递 channels_config 和 tools_config（对齐 nanobot）
    ...
    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step62：登记+完成自动移除）。
    ...
    def llm_runtime(self) -> LLMRuntime:
        """解析下一个 turn 使用的运行时（step62：refresh 为 no-op）。"""
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step62：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step62：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step62：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_compact(self, ctx: TurnContext) -> str:
        ...
        # step62：summary → pending_summary
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step62：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    def _prepare_message_media(
        ...
        """处理消息附件：非图片追加引用，图片路径保留（step56/step62）。

        step62：channels_config.extract_document_text 控制是否提取文档文本
        ...
        Returns:
            (更新后的 content, 图片路径列表)
        """
        # step62：extract_document_text 开关已贯通（当前不实现提取逻辑）
    ...
    def _build_initial_messages(
        ...
        # step62：处理 media 附件（非图片追加引用，图片追加占位符）
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step62.session.manager import replay_max_messages_for_context
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        step62：将 ``msg: InboundMessage`` 拆分为 ``channel``/``chat_id``/
        ...
        # step62：改用 for_turn（对齐 nanobot），支持 scoped channel 解析
        ...
        # step62：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step62：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step62：typed event 路径，带 stream_id
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step62：带 stream_id，resuming=False 时递增 segment。
        ...
        return AgentRunSpec(
            ...
            # step62：turn 级工具覆盖；None 时使用默认 self.registry
    ...
    async def _process_system_message(
        ...
        *,
        ...
        history = session.get_history(
            ...
            # step62：subagent follow-up 时 extend_to_user=True（对齐 nanobot）
    ...
    async def _run_agent_loop(
        ...
        *,
        ...
        step62：新增 ``hooks``（turn 级静态 hook）和 ``tools``（turn 级工具覆盖），
        ...
        Args:
            ...
            hooks: turn 级静态 hook（仅本 turn 有效）。step62 新增。
            hook_factories: turn 级 hook 工厂（仅本 turn 有效）。
            turn_scopes: turn 级 context manager 列表，运行期间进入，结束后退出。
            tools: turn 级工具覆盖；None 时使用默认 registry。step62 新增。
            ephemeral: 是否临时 turn（True 时 hook 链仅保留 progress hook）。
            run_extra_hooks_for_ephemeral: 临时 turn 是否也跑额外 hook。
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        # step62：不再重建 ctx.result，直接使用扁平字段
        ...
        _stream_spec = self._build_agent_spec(
            ...
            # step61：重建 spec 时也传入 ctx 的 turn 级配置，保证与 _run_agent_loop 内部一致
            hooks=ctx.hooks,
            hook_factories=ctx.hook_factories,
            tools=ctx.tools,
            ephemeral=ctx.ephemeral,
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        # step62：ctx.result → ctx.final_content
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # step62：tokens 从 ctx.usage 获取（_run_agent_loop 暂不填充，默认 0+0）
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step62：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step62：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step62：将 runtime_context 元数据设置到历史消息
    ...
    async def run_dream(self, tools: ToolRegistry | None = None) -> AgentRunResult | None:
        ...
        .. deprecated::
            将在 harness 阶段（step62）迁移到 ``process_direct(ephemeral=True)``。
            当前保留用于向后兼容。
        """
