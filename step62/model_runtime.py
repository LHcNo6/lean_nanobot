...
step62 为最小增量版：支持 model 切换、preset 选择、context window 调整；
provider 热刷新（refresh）和预设表解析留待 step62 配置层。
"""
...
class ModelRuntimeResolver:
    ...
    与 nanobot 的差异：
    - 暂无 model_presets 表 / provider_snapshot_loader（step62 补齐）
    ...
    def provider_signature(self) -> tuple[object, ...] | None:
        """当前 provider 快照签名（step62 透传 runtime.snapshot_signature）。"""
    ...
    def current(self, *, refresh: bool = False) -> LLMRuntime:
        ...
        Args:
            refresh: 是否刷新配置（step62 为 no-op，step62 补齐）。

        Returns:
            当前 LLMRuntime。
        """
        # step62：无 provider_snapshot_loader，refresh 暂不实现
        return self._runtime
    ...
    def select_preset(self, name: str | None) -> LLMRuntime:
        """选择命名预设为默认（step62 简化版：仅设置 model_preset 字段）。
