...
from step120.bus.events import InboundMessage
from step120.config.schema import ToolsConfig
from step120.context import RequestContext, ToolContext
from step120.hook import AgentHook, AgentHookContext
...
from step120.skills.loader import SkillsLoader
...
def _extract_fail_on_tool_error(config: Any) -> bool:
    """从配置安全提取子代理是否将工具错误升级为失败（step120）。
...
class SubagentManager:
    ...
    step120 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step120：manager 自己持有的长命令会话管理器，跨子代理共享
        ...
        # step120：从原始 config 提取禁用技能，构造子代理技能加载器
        self._disabled_skills = _extract_disabled_skills(config)
        # step120：从原始 config 提取子代理运行配置（对齐 nanobot 子代理运行配置传播）
        ...
        # step120：父会话墙钟超时解析回调（对齐 nanobot llm_wall_timeout_for_session）；
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        if self._provider is None:
            return
        # step120：从 origin 取出 workspace_scope，优先用以渲染 prompt 的工作区根；
        ...
        # step120：从 origin 重建请求上下文与 workspace 范围，注入 AgentRunSpec；
        ...
        # step120：以父会话 session_key 解析墙钟超时，写入 AgentRunSpec；
        ...
        # step120：子代理工具集每次 spawn 新建（对齐 nanobot per-spawn _build_tools）；
    ...
    def get_task_statuses(self) -> list[dict[str, Any]]:
        """返回所有子代理任务状态的快照（step120，对齐 nanobot self/my 可观测）。
