...
from step122.config.schema import Config, ToolsConfig
...
from step122.security.workspace_access import default_workspace_scope
from step122.subagent import SubagentManager, _flatten_tools_config
...
class TestRecursiveSpawnPrevention(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawned_subagent_cannot_recursive_spawn(self) -> None:
        ...
        step122 起 ``fail_on_tool_error`` 随配置生效（默认 True），子代理在
...
class TestSubagentRequestContextBinding(unittest.IsolatedAsyncioTestCase):
    """step122：子代理运行期 RequestContext / workspace_scope 绑定（契约 B1-B3）。"""
...
class TestSubagentRuntimeLimitSync(unittest.IsolatedAsyncioTestCase):
    """step122：子代理 llm_timeout_s 同步父会话墙钟超时策略（契约 D1+D2）。"""
