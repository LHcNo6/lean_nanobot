...
from step122.config import load_config, resolve_config_env_vars
from step122.context import ContextBuilder
from step122.llm import LLMRuntime
...
from step122.providers.factory import build_provider_snapshot
from step122.runner import AgentRunResult, AgentRunSpec
from step122.session import SessionManager
...
from step122.goal_state import runner_wall_llm_timeout_s
...
async def main():
    ...
    # step122：SubagentManager 不再复用主 registry —— 内部经 _build_tools()
    # 以 scope="subagent" 构建裁剪版工具集（对齐 nanobot），杜绝子代理
    # 递归 spawn 与调用 message/create_goal 等核心专属工具。
    subagent_manager = SubagentManager(
        ...
        # step122：把父会话墙钟超时策略同步到子代理（sustained-goal → 0.0 禁用，否则 None→env 默认）。
    ...
    async def _clock_runtime_context(request_context: Any):
        from step122.runtime_context import RuntimeContextBlock, wrap_runtime_context_lines
