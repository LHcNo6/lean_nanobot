...
from step122.bus.events import InboundMessage
from step122.config.schema import ToolsConfig
from step122.context import RequestContext, ToolContext
from step122.hook import AgentHook, AgentHookContext
...
from step122.skills.loader import SkillsLoader


# ---------------------------------------------------------------------------
# 子代理 announce 模板渲染（step122，对齐 nanobot agent/subagent_announce.md）
# ---------------------------------------------------------------------------
...
def _render_subagent_announce(label: str, status_text: str, task: str, result: str) -> str:
    """渲染子代理 announce 内容（step122，对齐 nanobot 模板）。
...
def _extract_fail_on_tool_error(config: Any) -> bool:
    """从配置安全提取子代理是否将工具错误升级为失败（step122）。
...
class SubagentManager:
    ...
    step122 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step122：manager 自己持有的长命令会话管理器，跨子代理共享
        ...
        # step122：从原始 config 提取禁用技能，构造子代理技能加载器
        self._disabled_skills = _extract_disabled_skills(config)
        # step122：从原始 config 提取子代理运行配置（对齐 nanobot 子代理运行配置传播）
        ...
        # step122：父会话墙钟超时解析回调（对齐 nanobot llm_wall_timeout_for_session）；
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        step122（G5，runtime 逐父同步）：从 ``origin["runtime"]`` 衍生
    ...
    async def _announce(self, task_id: str, label: str, task: str, result: str, origin: dict, status: str) -> None:
        # step122：对齐 nanobot，用模板渲染 announce（而非内联 f-string）
        ...
        # step122：对齐 nanobot，透传 origin_message_id 到 announce 元数据
    ...
    def get_task_statuses(self) -> list[dict[str, Any]]:
        """返回所有子代理任务状态的快照（step122，对齐 nanobot self/my 可观测）。
