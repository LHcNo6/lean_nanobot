"""定时任务工具：CronTool（step78 + step90 真实调度）。
...
step90 增强：CronScheduler 后台调度器，支持 every_seconds 和 at 任务的真实触发。
"""
...
from step103.schema import IntegerSchema, StringSchema, tool_parameters_schema
from step103.tool import Tool, ToolResult, tool_parameters
...
class _CronJob:
    ...
    next_run: float | None = None  # step90：下次触发时间戳（None 表示不调度）
...
class _CronStore:
    ...
    @property
    def jobs(self) -> dict[str, _CronJob]:
        """访问内部任务字典（调度器使用）。"""
        return self._jobs


# ---------------------------------------------------------------------------
# CronScheduler（step90：真实调度）
# ---------------------------------------------------------------------------
...
class CronScheduler:
    ...
    每秒检查一次任务，触发到期的任务并调用 on_trigger 回调。
    支持 every_seconds（周期性）和 at（一次性）任务。
    cron_expr 任务只存储不调度。
    """

    def __init__(
        self,
        on_trigger: Callable[[_CronJob], None] | None = None,
        check_interval: float = 1.0,
    ) -> None:
        """初始化调度器。

        Args:
            on_trigger: 任务触发时的回调函数，接收 _CronJob 参数。
            check_interval: 检查间隔秒数（默认 1 秒）。
        """
...
class CronTool(Tool):
    ...
    def create(cls, ctx: Any) -> Tool:
        ...
        if store is None and scheduler is not None:
            store = scheduler.store
        if store is None:
            store = _CronStore()
        return cls(cron_store=store, scheduler=scheduler)

    def __init__(self, cron_store: _CronStore, scheduler: CronScheduler | None = None):
        """初始化 CronTool。

        Args:
            cron_store: 定时任务存储。
            scheduler: 调度器实例（step90，可选）。
        """
    ...
    def _do_add(
        ...
        # step90：设置下次触发时间
        if every_seconds > 0:
            job.next_run = time.time() + every_seconds
        elif at:
            try:
                # 解析 ISO datetime 为时间戳
