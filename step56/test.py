...
from step56.autocompact import AutoCompact
from step56.bus import MessageBus
from step56.channel import BaseChannel
from step56.channels.cli import CliChannel
from step56.command import CommandContext, CommandRouter, normalize_command_text
from step56.consolidation import Consolidator, _consolidation_boundary
from step56.goal_state import *
from step56.helpers import estimate_message_tokens, estimate_prompt_tokens
from step56.llm import Runtime
from step56.memory import MemoryStore
from step56.context import ContextBuilder
from step56.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step56.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step56.llm import LLMResponse, ToolCallRequest
from step56.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step56.manager import ChannelManager
from step56.pairing import PAIRING_CODE_META_KEY, PairingStore
from step56.provider import LLMProvider
from step56.governance import ContextGovernanceConfig, ContextGovernor
from step56.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step56.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step56.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step56.session import Session, SessionManager
from step56.subagent import SubagentManager, SubagentStatus
from step56.tool import ToolRegistry, Tool
from step56.tools.long_task import CreateGoalTool, UpdateGoalTool
from step56.tools.echo import EchoTool
from step56.tools.spawn import SpawnTool
from step56.bus.events import StreamDeltaEvent
from step56.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step56.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step56.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step56.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step56.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step56.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step56.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step56.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step56.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step56.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step56.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step56：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step56：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step56.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step56.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step56.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step56.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step56.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step56.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step56.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step56.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step56.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step56.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step56.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step56.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step56.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step56：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_state_respond_ephemeral_hang_stop_reason(self):
        ...
        ctx = self._make_ctx(loop, ephemeral=True)
        ctx.result = mock.MagicMock()
        ctx.result.final_content = "hello"
        ctx.result.stop_reason = "stop"
        ctx.result.total_prompt_tokens = 10
        ctx.result.total_completion_tokens = 5
        ctx.stop_reason = "stop"
        event = await loop._state_respond(ctx)
        self.assertEqual(event, "ok")
        self.assertIsNotNone(ctx.outbound)
    ...
    def test_ephemeral_hook_only_progress(self):
        from step56.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step56.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep56StateTrace(unittest.IsolatedAsyncioTestCase):
    """step56：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step56.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step56.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep56SaveTurnEnhancements(unittest.TestCase):
    """step56：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step56.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep56CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step56：_state_command shortcut 持久化。"""
...
class Teststep56DropMalformedToolCalls(unittest.TestCase):
    """step56：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep56MalformedRetryMessages(unittest.TestCase):
    """step56：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep56MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step56：_request_malformed_fallback 降级无工具请求。"""
...
class Teststep56FinalizationRetryMessages(unittest.TestCase):
    """step56：_finalization_retry_messages 构造重试消息。"""
...
class Teststep56FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step56：空响应耗尽后发 finalization retry 集成测试。"""
...
class Teststep56ExtractReasoning(unittest.TestCase):
    """step56：extract_reasoning 剥离推理内容。"""

    def test_extract_reasoning_strips_think_tags(self):
        """<think> 标签从 content 中剥离，reasoning 被提取。"""
        from step56.helpers import extract_reasoning
...
class Teststep56ReasoningIntegration(unittest.IsolatedAsyncioTestCase):
    """step56：reasoning 提取 + emit_reasoning 集成测试。"""
...
class Teststep56FinalizeContent(unittest.IsolatedAsyncioTestCase):
    """step56：hook.finalize_content 被调用。"""
...
class Teststep56UsageTools(unittest.TestCase):
    """step56：usage 工具方法（_usage_dict / _usage_total / _merge_usage）。"""

    def test_usage_dict_converts_values(self):
        """_usage_dict 正确转换 usage 字典为 int，过滤非数字值。"""
        from step56.runner import AgentRunner
...
class Teststep56UsageEstimate(unittest.TestCase):
    """step56：_estimate_response_usage / _usage_or_estimate。"""
    ...
    def test_estimate_response_usage_returns_total_and_estimated(self):
        ...
        from step56.runner import AgentRunner, AgentRunSpec, LLMResponse
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hello"}],
            tools=_MockToolRegistry(),
            provider=_MockProvider(),
...
class Teststep56AccumulateUsage(unittest.TestCase):
    """step56：_accumulate_usage 累计所有键。"""

    def test_accumulate_usage_counts_all_keys(self):
        """_accumulate_usage 累计所有键（包括 total_tokens/estimated_tokens）。"""
        from step56.runner import AgentRunner, LLMResponse
...
class _step56CancelledRegistry:
...
class _step56TrackingHook(AgentHook):
    """记录工具执行 hook 调用。"""
    def __init__(self):
        self.before_calls = []
        self.after_calls = []
...
class Teststep56RunTool(unittest.IsolatedAsyncioTestCase):
    """step56：_run_tool 三元组返回 + hook 生命周期。"""

    def _make_ctx(self):
        from step56.hook import AgentHookContext
        return AgentHookContext(iteration=0, messages=[])

    def _make_spec(self, registry):
        from step56.runner import AgentRunSpec
        return AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
            tools=registry,
            provider=_MockProvider(),
    ...
    async def test_run_tool_returns_triple(self):
        ...
        from step56.runner import AgentRunner
        from step56.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step56EchoRegistry()
        spec = self._make_spec(registry)
        hook = _step56TrackingHook()
        ctx = self._make_ctx()
    ...
    async def test_run_tool_calls_on_error_hook(self):
        ...
        self.assertIsNone(error)  # step56 不设 fatal_error（留到 step56 fail_on_tool_error）
    ...
    async def test_run_tool_cancelled_error_propagates(self):
        ...
        from step56.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step56CancelledRegistry()
        spec = self._make_spec(registry)
        hook = _step56TrackingHook()
        ctx = self._make_ctx()
        with self.assertRaises(asyncio.CancelledError):
...
class Teststep56ExecuteToolBatch(unittest.IsolatedAsyncioTestCase):
    """step56：_execute_tool_batch 三元组返回。"""

    async def test_execute_tool_batch_returns_triple(self):
        """_execute_tool_batch 返回 (results, events, fatal_error)。"""
        from step56.runner import AgentRunner, AgentRunSpec
        from step56.governance import ContextGovernanceConfig
        from step56.hook import AgentHookContext, AgentHook
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step56EchoRegistry("ok")
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
...
class Teststep56RunCancelledError(unittest.IsolatedAsyncioTestCase):
    """step56：run() 中 CancelledError 分离。"""

    async def test_run_cancelled_error_no_on_error(self):
        """run() 中 CancelledError 不调 on_error。"""
        from step56.runner import AgentRunner, AgentRunSpec
        from step56.hook import AgentHook
...
class Teststep56SSRFDetection(unittest.TestCase):
    """step56：SSRF 检测方法。"""
...
class Teststep56ClassifyViolation(unittest.TestCase):
    """step56：_classify_violation 统一分类。"""
...
class Teststep56RepeatedLookup(unittest.TestCase):
    """step56：重复外部查找和 workspace 违规检测。"""

    def test_repeated_external_lookup_blocks_after_2(self):
        """重复外部查找 2 次后阻断。"""
        from step56.helpers import repeated_external_lookup_error
        counts = {}
        # 前 2 次不阻断
    ...
    def test_repeated_workspace_violation_escalates_after_2(self):
        ...
        from step56.helpers import repeated_workspace_violation_error
        counts = {}
        # 前 2 次不升级
...
class Teststep56RunToolSecurity(unittest.IsolatedAsyncioTestCase):
    """step56：_run_tool 中安全检测集成。"""
...
class TestStep52SpecFields(unittest.TestCase):
    ...
    def test_tool_events_default_empty(self):
        ...
        from step56.runner import AgentRunResult
        result = AgentRunResult(final_content="hi", messages=[])
        self.assertEqual(result.tool_events, [])


class Teststep56FailOnToolError(unittest.IsolatedAsyncioTestCase):
    """step56：fail_on_tool_error 终止 turn。"""
...
class Teststep56ToolEvents(unittest.IsolatedAsyncioTestCase):
    """step56：tool_events 收集。"""
...
class Teststep56SpecFields(unittest.TestCase):
    """step56：AgentRunSpec 新字段。"""
...
class Teststep56IncrementalThinkExtractor(unittest.IsolatedAsyncioTestCase):
    """step56：IncrementalThinkExtractor 基本功能。"""

    async def test_feed_extracts_think(self):
        """feed 提取 <think> 块中的新文本并 emit。"""
        from step56.helpers import IncrementalThinkExtractor
        emitted = []
        async def emit(text):
            emitted.append(text)
...
class Teststep56StreamingThinking(unittest.IsolatedAsyncioTestCase):
    """step56：流式 thinking 提取。"""
...
class TestStep54StreamSegmentation(unittest.IsolatedAsyncioTestCase):
    ...
    def test_stream_delta_typed_event_has_stream_id(self):
        ...
        from step56.bus.outbound_events import StreamDeltaEvent as TypedDelta
...
class TestStep55ModelRuntimeResolver(unittest.TestCase):
    """step56：ModelRuntimeResolver 核心功能。"""

    def _make_runtime(self, model="gpt-4", preset=None, sig=None):
        from step56.llm import GenerationSettings, LLMRuntime
        return LLMRuntime(
            provider=object(),
            model=model,
    ...
    def test_runtime_property_returns_initial(self):
        from step56.model_runtime import ModelRuntimeResolver
...
class TestStep55AgentLoopRuntime(unittest.TestCase):
    """step56：AgentLoop runtime 委托给 ModelRuntimeResolver。"""
...
class TestStep56DocumentUtils(unittest.TestCase):
    """step56：utils/document.py 附件处理。"""

    def test_is_image_file_png(self):
        from step56.utils.document import is_image_file
    ...
    def test_reference_non_image_attachments_separates(self):
        from step56.utils.document import reference_non_image_attachments
...
class TestStep56ImagePlaceholder(unittest.TestCase):
    """step56：image_placeholder_text。"""

    def test_with_path(self):
        from step56.helpers import image_placeholder_text
...
class TestStep56PrepareMessageMedia(unittest.TestCase):
    """step56：AgentLoop._prepare_message_media。"""
