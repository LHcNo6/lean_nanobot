...
from step56.bus.outbound_events import (
    RetryWaitEvent,
...
from step56.bus.progress import build_bus_progress_callback
from step56.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step56.autocompact import AutoCompact
from step56.command import CommandContext, CommandRouter, register_builtin_commands
from step56.consolidation import Consolidator
from step56.context import ContextBuilder, RequestContext
...
from step56.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step56.model_runtime import ModelRuntimeResolver
from step56.memory import MemoryStore
from step56.pairing import PairingStore
from step56.runtime_context import (
...
from step56.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step56.security.workspace_access import WorkspaceScopeResolver
from step56.session import Session, SessionManager
from step56.session import turn_continuation
from step56.session.history_visibility import HIDDEN_HISTORY_META
from step56.session.keys import UNIFIED_SESSION_KEY
from step56.subagent import SubagentManager
from step56.tool import ToolRegistry
from step56.context import ToolContext
from step56.loader import ToolLoader
from step56.helpers import image_placeholder_text, truncate_text
from step56.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step56 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    stop_reason: str = ""
    # step32：runner 结果的错误文案和注入标记，供 _state_respond / 事件使用。
    error: str | None = None
    ...
    # step56：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step56：跟踪后台任务，shutdown 时 drain
        ...
        # step56：runtime 管理委托给 ModelRuntimeResolver
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step56.providers.factory import make_provider
    ...
    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step56：登记+完成自动移除）。
    ...
    def llm_runtime(self) -> LLMRuntime:
        """解析下一个 turn 使用的运行时（step56：refresh 为 no-op）。"""
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step56：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step56：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step56：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_compact(self, ctx: TurnContext) -> str:
        ...
        ctx.summary = pending
        # step33：consolidation 调用移到 _state_build（带 replay_max_messages），
        # 对齐 nanobot：_state_compact 只做 auto_compact，不做 token 预算压缩。
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step56：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    def _prepare_message_media(
        ...
        """处理消息附件：非图片追加引用，图片路径保留（step56）。

        step56 简化版：总是调用 reference_non_image_attachments（不提取
    ...
    def _build_initial_messages(
        ...
        # step56：处理 media 附件（非图片追加引用，图片追加占位符）
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step56.session.manager import replay_max_messages_for_context
        ...
        ctx.initial_messages = self._build_initial_messages(
            ctx.msg, ctx.session, ctx.history, ctx.summary,
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        # step56：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step56：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step56：typed event 路径，带 stream_id
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step56：带 stream_id，resuming=False 时递增 segment。
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        # 重建 ctx.result（_state_save / _state_respond 依赖它）。
        ...
        # step32：错误文案落盘到 ctx（_run_agent_loop 已记录日志）。
        ctx.error = ctx.result.error
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # tokens 字段在调用方填充（_assemble_outbound 不持有 ctx.result）
        if outbound is not None:
            outbound.metadata["tokens"] = (
                f"{ctx.result.total_prompt_tokens}+{ctx.result.total_completion_tokens}"
            )
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step56：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step56：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step56：将 runtime_context 元数据设置到历史消息
