...
from step124.schema import StringSchema, tool_parameters_schema
...
_READ_ONLY = frozenset({
    ...
    "subagents",  # step124：子代理状态只读可观测，不可经 my 工具修改
})
...
class MyTool(Tool):
    ...
    def _get_runtime_value(self, key: str) -> Any:
        ...
        step124 增强：支持点分嵌套路径（如 config.exec.timeout）和 agent key。
        ...
        if key == "subagents":
            # step124：可观测子代理状态（对齐 nanobot self/my 的 subagents key）
    ...
    def _has_key(self, key: str) -> bool:
        ...
        step124 增强：新增可观测 key ``subagents``。
        """
