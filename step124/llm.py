...
class LLMRuntime:
    ...
    def with_generation_overrides(
        ...
        """返回 generations 参数被覆写后的新运行设置（原对象不可变）。

        step124（G7）：供 spawn 覆写子代理 temperature（及可选 max_tokens）使用，
        对齐 nanobot ``LLMRuntime.with_generation_overrides``。仅覆写非空参数，
        provider / model / context_window_tokens 等其余字段沿用原值。
        """
        gen = self.generation
        return LLMRuntime(
