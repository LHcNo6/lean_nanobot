...
from step124.config import load_config, resolve_config_env_vars
from step124.context import ContextBuilder
from step124.llm import LLMRuntime
...
from step124.providers.factory import build_provider_snapshot
from step124.runner import AgentRunResult, AgentRunSpec
...
from step124.goal_state import runner_wall_llm_timeout_s
...
async def main():
    ...
    # step124：SubagentManager 不再复用主 registry —— 内部经 _build_tools()
    ...
    subagent_manager = SubagentManager(
        ...
        # step124：把父会话墙钟超时策略同步到子代理（sustained-goal → 0.0 禁用，否则 None→env 默认）。
    ...
    async def _clock_runtime_context(request_context: Any):
        from step124.runtime_context import RuntimeContextBlock, wrap_runtime_context_lines
