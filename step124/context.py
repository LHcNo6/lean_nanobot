...
class ToolContext:
    ...
    exec_session_manager: Any = None  # step73：ExecSessionManager 实例，管理长运行命令会话
    cli_app_manager: Any = None  # step83：CliAppManager 实例，管理已注册 CLI 应用白名单
    cron_store: Any = None  # step78：定时任务存储
    cron_scheduler: Any = None  # step90：定时任务调度器
    agent_loop: Any = None  # step124：AgentLoop 引用（MyTool 自省）
    workspace_sandbox: Any | None = None  # step124（G9）：宿主 workspace 限制状态（WorkspaceSandboxStatus）
...
class ContextBuilder:
    ...
    def skills(self) -> Any:
        ...
        if self._skills is None:
            from step124.skills import SkillsLoader
    ...
    @property
    def memory(self) -> Any:
        """惰性构建（并缓存）MemoryStore，供长期记忆注入。

        首次访问时创建 MemoryStore 实例（会触发 ensure_dir 创建 memory/ 目录），
        后续访问直接返回缓存实例。
        """
        if self._memory is None:
            from step124.memory import MemoryStore

            self._memory = MemoryStore(workspace=self.workspace)
        return self._memory
    ...
    def build_system_prompt(
        ...
        ephemeral turn 传 False——临时 turn 不读取跨会话记忆。
        step93：注入 MEMORY.md 长期记忆。
        step124：注入近期历史（# Recent History 段），按 session_key 过滤。
        ...
        # 长期记忆注入（MEMORY.md），对齐 nanobot get_memory_context()
        if include_memory_recent_history:
            memory_context = self.memory.get_memory_context()
            if memory_context:
                parts.append(memory_context)

            # 近期历史注入（# Recent History 段）
