...
from step26.bus.outbound_events import (
    RetryWaitEvent,
    StreamedResponseEvent,
    outbound_message_for_event,
)
from step26.bus.progress import build_bus_progress_callback
from step26.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step26.autocompact import AutoCompact
from step26.command import CommandContext, CommandRouter, register_builtin_commands
from step26.consolidation import Consolidator
from step26.context import ContextBuilder
from step26.goal_state import goal_state_runtime_lines, sustained_goal_active
from step26.hook import AgentHook, AgentHookContext, CompositeHook
from step26.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step26.memory import MemoryStore
from step26.pairing import PairingStore
from step26.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
...
from step26.subagent import SubagentManager
from step26.tool import ToolRegistry
...
class AgentLoop:
    ...
    def __init__(
        ...
        self.config = config  # step26: 装配时的 Config（供工具上下文解析配置）
        ...
        # step26: 进程内运行时事件总线（turn 生命周期观测）。外部订阅者可
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step26.providers.factory import make_provider
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        try:
            ...
            # step26: turn 生命周期事件——无论成败都标记完成并将 run status
    ...
    async def _state_restore(self, ctx: TurnContext) -> str:
        ...
        # step26: turn 生命周期事件——会话 turn 已加载 session，协议本文将构建上下文。
    ...
    async def _process_system_message(
        ...
        *,
        ...
        # step26: 系统通道同样装配 bus 回调（progress / retry_wait）。
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        # step26: 惰性构建 bus 回调（progress / retry_wait），对齐 nanobot
        # ``_state_build`` 的默认回调装配。
        if ctx.on_progress is None:
            ctx.on_progress = await self._build_bus_progress_callback(ctx.msg)
        if ctx.on_retry_wait is None:
            ctx.on_retry_wait = await self._build_retry_wait_callback(ctx.msg)
        # step26: 可见运行状态 + 记录该 turn 使用的 runtime（随 TurnCompleted 派发）。
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        ...
        # step26: 记录 turn 延迟，随 TurnCompleted 派发给运行时事件订阅者。
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # step26: 流式已交付时在最终消息上挂 StreamedResponseEvent，让通道
