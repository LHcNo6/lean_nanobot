...
from step102.memory import MemoryStore, _ARCHIVE_SUMMARY_MAX_CHARS, _RAW_ARCHIVE_MAX_CHARS
...
_CONSOLIDATOR_SYSTEM_PROMPT = (
    ...
    "Dream handles cross-file deduplication later.\n"
    "\n"
...
class Consolidator:
    ...
    def _truncate_to_token_budget(self, text: str, runtime: Runtime) -> str:
        """将文本截断到 consolidation LLM 的 token 预算内。

        使用 ``truncate_text_to_tokens`` 精确计算 token 数（tiktoken 不可用
        时回退到 char 估算）。budget <= 0 时回退到 ``_RAW_ARCHIVE_MAX_CHARS``。
        """
    ...
    async def archive(self, messages: list[dict[str, Any]], *, runtime: Runtime, session_key: str | None = None, summary_messages: list[dict[str, Any]] | None = None) -> str | None:
        ...
        from step102.runtime_context import public_history_messages
        ...
        try:
            ...
            if getattr(response, "finish_reason", None) == "error":
                raise RuntimeError(f"LLM returned error: {getattr(response, 'content', '')}")
    ...
    async def maybe_consolidate_by_tokens(self, session: Session, *, runtime: Runtime, replay_max_messages: int | None = None) -> None:
        ...
        async with lock:
            ...
            try:
                estimated, _source = self.estimate_session_prompt_tokens(session, runtime=runtime)
            except Exception:
                estimated = 0
            if estimated <= 0:
