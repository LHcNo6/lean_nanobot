...
from step46.autocompact import AutoCompact
from step46.bus import MessageBus
from step46.channel import BaseChannel
from step46.channels.cli import CliChannel
from step46.command import CommandContext, CommandRouter, normalize_command_text
from step46.consolidation import Consolidator, _consolidation_boundary
from step46.goal_state import *
from step46.helpers import estimate_message_tokens, estimate_prompt_tokens
from step46.llm import Runtime
from step46.memory import MemoryStore
from step46.context import ContextBuilder
from step46.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step46.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step46.llm import LLMResponse, ToolCallRequest
from step46.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step46.manager import ChannelManager
from step46.pairing import PAIRING_CODE_META_KEY, PairingStore
from step46.provider import LLMProvider
from step46.governance import ContextGovernanceConfig, ContextGovernor
from step46.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step46.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step46.runner import AgentRunSpec, AgentRunner
from step46.session import Session, SessionManager
from step46.subagent import SubagentManager, SubagentStatus
from step46.tool import ToolRegistry, Tool
from step46.tools.long_task import CreateGoalTool, UpdateGoalTool
from step46.tools.echo import EchoTool
from step46.tools.spawn import SpawnTool
from step46.bus.events import StreamDeltaEvent
from step46.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step46.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step46.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step46.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step46.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step46.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step46.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step46.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step46.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step46.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step46.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step46：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step46：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step46.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step46.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step46.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step46.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step46.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step46.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step46.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step46.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step46.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step46.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step46.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step46.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step46.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step46：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step46.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step46.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep46StateTrace(unittest.IsolatedAsyncioTestCase):
    """step46：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step46.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step46.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep46SaveTurnEnhancements(unittest.TestCase):
    """step46：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step46.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep46CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step46：_state_command shortcut 持久化。"""
...
class TestStep46DropMalformedToolCalls(unittest.TestCase):
    """step46：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class TestStep46MalformedRetryMessages(unittest.TestCase):
    """step46：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class TestStep46MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step46：_request_malformed_fallback 降级无工具请求。"""
