...
class ModelPresetConfig(Base):
    ...
    def to_generation_settings(self) -> Any:
        from step115.llm import GenerationSettings
...
class ProvidersConfig(Base):
    ...
    def convert_extra_providers(self) -> "ProvidersConfig":
        ...
        from step115.providers.registry import find_by_name
...
class CliAppSpec(Base):
    """单个 CLI 应用的声明（step115）。

    Attributes:
        name: 注册名（``run_cli_app`` 用此名调用）。
        command: 实际可执行命令（argv 首段；Windows 上含空格需自行加引号）。
        description: 应用描述（可选）。
    """
...
class CliAppsConfig(Base):
    """cli_apps 配置段（step115，对齐 nanobot ``Config.cli_apps`` 最小形态）。"""
...
class Config(Base):
    ...
    cli_apps: CliAppsConfig = Field(default_factory=CliAppsConfig)  # step115
    ...
    def _match_provider(
        ...
        from step115.providers.registry import PROVIDERS, find_by_name, find_by_model
