...
from step59.bus.outbound_events import (
    RetryWaitEvent,
...
from step59.bus.progress import build_bus_progress_callback
from step59.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step59.autocompact import AutoCompact
from step59.command import CommandContext, CommandRouter, register_builtin_commands
from step59.consolidation import Consolidator
from step59.context import ContextBuilder, RequestContext
...
from step59.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step59.model_runtime import ModelRuntimeResolver
from step59.memory import MemoryStore
from step59.pairing import PairingStore
from step59.runtime_context import (
...
from step59.session import turn_continuation
from step59.session.history_visibility import HIDDEN_HISTORY_META
from step59.session.keys import UNIFIED_SESSION_KEY
from step59.subagent import SubagentManager
from step59.tool import ToolRegistry
from step59.context import ToolContext
from step59.loader import ToolLoader
from step59.helpers import image_placeholder_text, truncate_text
from step59.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step59 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step59：summary 改名为 pending_summary（对齐 nanobot）
    ...
    stop_reason: str = ""
    # step59：移除 error 字段（错误通过 final_content + stop_reason 传递）
    had_injections: bool = False
    # step59：usage 字典（tokens 信息）
    ...
    # step59：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step59：跟踪后台任务，shutdown 时 drain
        ...
        # step59：runtime 管理委托给 ModelRuntimeResolver
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step59.providers.factory import make_provider
    ...
    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step59：登记+完成自动移除）。
    ...
    def llm_runtime(self) -> LLMRuntime:
        """解析下一个 turn 使用的运行时（step59：refresh 为 no-op）。"""
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step59：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step59：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step59：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_compact(self, ctx: TurnContext) -> str:
        ...
        # step59：summary → pending_summary
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step59：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    def _prepare_message_media(
        ...
        """处理消息附件：非图片追加引用，图片路径保留（step59）。

        step59 简化版：总是调用 reference_non_image_attachments（不提取
    ...
    def _build_initial_messages(
        ...
        # step59：处理 media 附件（非图片追加引用，图片追加占位符）
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step59.session.manager import replay_max_messages_for_context
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        # step59：改用 for_turn（对齐 nanobot），支持 scoped channel 解析
        ...
        # step59：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step59：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step59：typed event 路径，带 stream_id
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step59：带 stream_id，resuming=False 时递增 segment。
    ...
    async def _process_system_message(
        ...
        *,
        ...
        history = session.get_history(
            ...
            # step59：subagent follow-up 时 extend_to_user=True（对齐 nanobot）
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        # step59：不再重建 ctx.result，直接使用扁平字段
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        # step59：ctx.result → ctx.final_content
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # step59：tokens 从 ctx.usage 获取（_run_agent_loop 暂不填充，默认 0+0）
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step59：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step59：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step59：将 runtime_context 元数据设置到历史消息
    ...
    async def run_dream(self, tools: ToolRegistry | None = None) -> AgentRunResult | None:
        ...
        .. deprecated::
            将在 harness 阶段（step59）迁移到 ``process_direct(ephemeral=True)``。
            当前保留用于向后兼容。
        """
