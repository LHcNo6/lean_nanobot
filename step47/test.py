...
from step47.autocompact import AutoCompact
from step47.bus import MessageBus
from step47.channel import BaseChannel
from step47.channels.cli import CliChannel
from step47.command import CommandContext, CommandRouter, normalize_command_text
from step47.consolidation import Consolidator, _consolidation_boundary
from step47.goal_state import *
from step47.helpers import estimate_message_tokens, estimate_prompt_tokens
from step47.llm import Runtime
from step47.memory import MemoryStore
from step47.context import ContextBuilder
from step47.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step47.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step47.llm import LLMResponse, ToolCallRequest
from step47.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step47.manager import ChannelManager
from step47.pairing import PAIRING_CODE_META_KEY, PairingStore
from step47.provider import LLMProvider
from step47.governance import ContextGovernanceConfig, ContextGovernor
from step47.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step47.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step47.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step47.session import Session, SessionManager
from step47.subagent import SubagentManager, SubagentStatus
from step47.tool import ToolRegistry, Tool
from step47.tools.long_task import CreateGoalTool, UpdateGoalTool
from step47.tools.echo import EchoTool
from step47.tools.spawn import SpawnTool
from step47.bus.events import StreamDeltaEvent
from step47.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step47.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step47.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step47.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step47.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step47.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step47.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step47.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step47.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step47.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step47.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step47：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step47：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step47.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step47.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step47.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step47.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step47.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step47.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step47.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step47.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step47.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step47.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step47.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step47.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step47.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step47：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step47.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step47.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep47StateTrace(unittest.IsolatedAsyncioTestCase):
    """step47：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step47.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step47.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep47SaveTurnEnhancements(unittest.TestCase):
    """step47：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step47.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep47CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step47：_state_command shortcut 持久化。"""
...
class Teststep47DropMalformedToolCalls(unittest.TestCase):
    """step47：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep47MalformedRetryMessages(unittest.TestCase):
    """step47：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep47MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step47：_request_malformed_fallback 降级无工具请求。"""
...
class TestStep47FinalizationRetryMessages(unittest.TestCase):
    """step47：_finalization_retry_messages 构造重试消息。"""
...
class TestStep47FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step47：空响应耗尽后发 finalization retry 集成测试。"""
