...
from step47.hook import AgentHook, AgentHookContext, AgentRunHookContext
from step47.llm import LLMResponse
from step47.provider import LLMProvider
from step47.tool import Tool, ToolRegistry
...
class AgentRunner:
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        from step47.security.workspace_access import bind_workspace_scope, reset_workspace_scope
        from step47.tools.file_state import FileStates, bind_file_states, reset_file_states
    ...
    async def _request_model(
        ...
        *,
        ...
        # step47：malformed tool_call 处理——递归重试 + 降级无工具
    ...
    def _drop_malformed_tool_calls(
        ...
        """step47：返回 (dropped_count, all_dropped, original_finish_reason)。
    ...
    def _malformed_tool_call_retry_messages(
        ...
        """step47：构造 malformed tool_call 重试提示消息（保留原 assistant 文本）。"""
    ...
    async def _request_malformed_fallback(
        ...
        """step47：malformed_retry 仍失败时降级为无工具请求（直接调 provider）。"""
    ...
    def _finalization_retry_messages(
        ...
        """step47：构造 finalization 重试消息（无工具，让模型基于对话生成最终答案）。"""
        retry_messages = list(messages)
    ...
    def _append_injected_messages(
        ...
        from step47.session.history_visibility import is_hidden_history_message
    ...
    async def _run_loop(
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.tool_calls and response.finish_reason == "tool_calls":
                # step47：malformed 已在 _request_model 中处理，直接用 response
