...
from step55.bus.outbound_events import (
    RetryWaitEvent,
...
from step55.bus.progress import build_bus_progress_callback
from step55.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step55.autocompact import AutoCompact
from step55.command import CommandContext, CommandRouter, register_builtin_commands
from step55.consolidation import Consolidator
from step55.context import ContextBuilder, RequestContext
...
from step55.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step55.model_runtime import ModelRuntimeResolver
from step55.memory import MemoryStore
from step55.pairing import PairingStore
from step55.runtime_context import (
...
from step55.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step55.security.workspace_access import WorkspaceScopeResolver
from step55.session import Session, SessionManager
from step55.session import turn_continuation
from step55.session.history_visibility import HIDDEN_HISTORY_META
from step55.session.keys import UNIFIED_SESSION_KEY
from step55.subagent import SubagentManager
from step55.tool import ToolRegistry
from step55.context import ToolContext
from step55.loader import ToolLoader
from step55.helpers import truncate_text
from step55.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step55 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step55：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step55：跟踪后台任务，shutdown 时 drain
        ...
        # step55：runtime 管理委托给 ModelRuntimeResolver
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step55.providers.factory import make_provider
    ...
    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step55：登记+完成自动移除）。
    ...
    def llm_runtime(self) -> LLMRuntime:
        """解析下一个 turn 使用的运行时（step55：refresh 为 no-op）。"""
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step55：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step55：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step55：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step55：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step55.session.manager import replay_max_messages_for_context
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        # step55：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step55：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step55：typed event 路径，带 stream_id
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step55：带 stream_id，resuming=False 时递增 segment。
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step55：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step55：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step55：将 runtime_context 元数据设置到历史消息
