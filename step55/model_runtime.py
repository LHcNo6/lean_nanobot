...
step55 为最小增量版：支持 model 切换、preset 选择、context window 调整；
...
class ModelRuntimeResolver:
    ...
    def provider_signature(self) -> tuple[object, ...] | None:
        """当前 provider 快照签名（step55 透传 runtime.snapshot_signature）。"""
    ...
    def current(self, *, refresh: bool = False) -> LLMRuntime:
        ...
        Args:
            refresh: 是否刷新配置（step55 为 no-op，step60 补齐）。

        Returns:
            当前 LLMRuntime。
        """
        # step55：无 provider_snapshot_loader，refresh 暂不实现
        return self._runtime
    ...
    def select_preset(self, name: str | None) -> LLMRuntime:
        """选择命名预设为默认（step55 简化版：仅设置 model_preset 字段）。
