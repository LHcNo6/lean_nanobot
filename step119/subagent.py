...
from step119.context import RequestContext, ToolContext
from step119.hook import AgentHook, AgentHookContext
...
from step119.skills.loader import SkillsLoader
...
class SubagentManager:
    ...
    step119 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step119：manager 自己持有的长命令会话管理器，跨子代理共享
        ...
        # step119：从原始 config 提取禁用技能，构造子代理技能加载器
        ...
        # step119：父会话墙钟超时解析回调（对齐 nanobot llm_wall_timeout_for_session）；
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        if self._provider is None:
            return
        # step119：从 origin 取出 workspace_scope，优先用以渲染 prompt 的工作区根；
        ...
        # step119：从 origin 重建请求上下文与 workspace 范围，注入 AgentRunSpec；
        ...
        # step119：以父会话 session_key 解析墙钟超时，写入 AgentRunSpec；
