...
from step107.channel import BaseChannel
from step107.channels.cli import CliChannel
from step107.command import CommandContext, CommandRouter, normalize_command_text
from step107.consolidation import Consolidator, _consolidation_boundary
from step107.goal_state import *
from step107.helpers import estimate_message_tokens, estimate_prompt_tokens
from step107.llm import Runtime
from step107.memory import MemoryStore
from step107.context import ContextBuilder
from step107.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step107.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step107.llm import LLMResponse, ToolCallRequest
from step107.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step107.manager import ChannelManager
from step107.pairing import PAIRING_CODE_META_KEY, PairingStore
from step107.provider import LLMProvider
from step107.governance import ContextGovernanceConfig, ContextGovernor
from step107.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step107.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step107.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step107.session import Session, SessionManager
from step107.subagent import SubagentManager, SubagentStatus
from step107.tool import ToolRegistry, Tool
from step107.tools.long_task import CreateGoalTool, UpdateGoalTool
from step107.tools.echo import EchoTool
from step107.tools.spawn import SpawnTool
from step107.bus.events import StreamDeltaEvent
from step107.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step107.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step107.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step107.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step107.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step107.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step107.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step107.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step107.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step107.context import reset_request_context
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step107.tool import tool_parameters
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step107.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step107.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step107.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step107.context import RequestContext
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step107.command.builtin import register_builtin_commands
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step107.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class Teststep64StateTrace(unittest.IsolatedAsyncioTestCase):
    ...
    def test_state_trace_entry_creation(self):
        from step107.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step107.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep64SaveTurnEnhancements(unittest.TestCase):
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step107.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep64ExtractReasoning(unittest.TestCase):
    ...
    def test_extract_reasoning_strips_think_tags(self):
        ...
        from step107.helpers import extract_reasoning
...
class Teststep64UsageTools(unittest.TestCase):
    ...
    def test_usage_dict_converts_values(self):
        ...
        from step107.runner import AgentRunner
...
class Teststep64UsageEstimate(unittest.TestCase):
    ...
    def test_estimate_response_usage_returns_total_and_estimated(self):
        ...
        from step107.runner import AgentRunner, AgentRunSpec, LLMResponse
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hello"}],
            tools=_MockToolRegistry(),
            provider=_MockProvider(),
...
class Teststep64AccumulateUsage(unittest.TestCase):
    ...
    def test_accumulate_usage_counts_all_keys(self):
        ...
        from step107.runner import AgentRunner, LLMResponse
...
class Teststep64RunTool(unittest.IsolatedAsyncioTestCase):
    ...
    def _make_ctx(self):
        from step107.hook import AgentHookContext
        return AgentHookContext(iteration=0, messages=[])

    def _make_spec(self, registry):
        from step107.runner import AgentRunSpec
        return AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
            tools=registry,
            provider=_MockProvider(),
    ...
    async def test_run_tool_returns_triple(self):
        ...
        from step107.runner import AgentRunner
        from step107.governance import ContextGovernanceConfig
...
class Teststep64ExecuteToolBatch(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_execute_tool_batch_returns_triple(self):
        ...
        from step107.runner import AgentRunner, AgentRunSpec
        from step107.governance import ContextGovernanceConfig
        from step107.hook import AgentHookContext, AgentHook
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
...
class Teststep64RunCancelledError(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_cancelled_error_no_on_error(self):
        ...
        from step107.runner import AgentRunner, AgentRunSpec
        from step107.hook import AgentHook
...
class Teststep64RepeatedLookup(unittest.TestCase):
    ...
    def test_repeated_external_lookup_blocks_after_2(self):
        """重复外部查找 2 次后阻断。"""
        from step107.helpers import repeated_external_lookup_error
        counts = {}
        # 前 2 次不阻断
    ...
    def test_repeated_workspace_violation_escalates_after_2(self):
        ...
        from step107.helpers import repeated_workspace_violation_error
        counts = {}
        # 前 2 次不升级
...
class TestStep52SpecFields(unittest.TestCase):
    ...
    def test_tool_events_default_empty(self):
        ...
        from step107.runner import AgentRunResult
...
class Teststep64IncrementalThinkExtractor(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_feed_extracts_think(self):
        ...
        from step107.helpers import IncrementalThinkExtractor
        emitted = []
        async def emit(text):
            emitted.append(text)
...
class TestStep54GoalContinueCallable(unittest.TestCase):
    ...
    def test_callable_goal_continue_message_invoked(self):
        ...
        from step107.runner import AgentRunSpec, AgentRunner
        runner = AgentRunner()
        calls = []
...
class TestStep54BackgroundTasks(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_schedule_background_tracks_task(self):
        ...
        import asyncio
        from step107.loop import AgentLoop
...
class TestStep54StreamSegmentation(unittest.IsolatedAsyncioTestCase):
    ...
    def test_stream_delta_typed_event_has_stream_id(self):
        ...
        from step107.bus.outbound_events import StreamDeltaEvent as TypedDelta
...
class TestStep55ModelRuntimeResolver(unittest.TestCase):
    ...
    def _make_runtime(self, model="gpt-4", preset=None, sig=None):
        from step107.llm import GenerationSettings, LLMRuntime
        return LLMRuntime(
            provider=object(),
            model=model,
...
class TestStep56DocumentUtils(unittest.TestCase):
    ...
    def test_is_image_file_png(self):
        from step107.utils.document import is_image_file
...
class TestStep56ImagePlaceholder(unittest.TestCase):
    ...
    def test_with_path(self):
        from step107.helpers import image_placeholder_text
...
class TestStep57TurnContextFields(unittest.TestCase):
    ...
    def test_turn_context_no_result_field(self):
        ...
        from step107.loop import TurnContext
        ctx = TurnContext(
            msg=InboundMessage(content="hi"),
            session_key="test",
        )
...
class TestStep58ModelErrorPlaceholder(unittest.TestCase):
    ...
    def test_append_placeholder(self):
        from step107.runner import AgentRunner, _PERSISTED_MODEL_ERROR_PLACEHOLDER
        messages = [{"role": "user", "content": "hi"}]
    ...
    def test_placeholder_constant_text(self):
        from step107.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
...
class TestStep58IsToolErrorResult(unittest.TestCase):
    ...
    def test_error_result(self):
        from step107.tool import ToolResult, is_tool_error_result
    ...
    def test_non_tool_result(self):
        from step107.tool import is_tool_error_result
...
class Teststep64ChannelsConfigExtractDocumentText(unittest.TestCase):
    ...
    def test_default_true(self):
        from step107.config.schema import ChannelsConfig
        cfg = ChannelsConfig()
...
class Teststep64ToolsConfigWebExec(unittest.TestCase):
    ...
    def test_web_default(self):
        from step107.config.schema import ToolsConfig
        cfg = ToolsConfig()
