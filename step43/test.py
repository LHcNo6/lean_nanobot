...
from step43.autocompact import AutoCompact
from step43.bus import MessageBus
from step43.channel import BaseChannel
from step43.channels.cli import CliChannel
from step43.command import CommandContext, CommandRouter, normalize_command_text
from step43.consolidation import Consolidator, _consolidation_boundary
from step43.goal_state import *
from step43.helpers import estimate_message_tokens, estimate_prompt_tokens
from step43.llm import Runtime
from step43.memory import MemoryStore
from step43.context import ContextBuilder
from step43.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step43.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step43.llm import LLMResponse, ToolCallRequest
from step43.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step43.manager import ChannelManager
from step43.pairing import PAIRING_CODE_META_KEY, PairingStore
from step43.provider import LLMProvider
from step43.governance import ContextGovernanceConfig, ContextGovernor
from step43.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step43.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step43.subagent import SubagentManager, SubagentStatus
from step43.tool import ToolRegistry, Tool
from step43.tools.long_task import CreateGoalTool, UpdateGoalTool
from step43.tools.echo import EchoTool
from step43.tools.spawn import SpawnTool
from step43.bus.events import StreamDeltaEvent
from step43.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step43.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step43.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step43.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step43.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step43.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step43.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step43.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step43.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step43.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step43.context import reset_request_context
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step43.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step43.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step43.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step43.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step43.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step43.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step43.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step43.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step43.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step43.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step43.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step43.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step43.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step43.tools.message import MessageTool
        loop, _ = self._make_loop()
