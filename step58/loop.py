...
from step58.bus.outbound_events import (
    RetryWaitEvent,
...
from step58.bus.progress import build_bus_progress_callback
from step58.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step58.autocompact import AutoCompact
from step58.command import CommandContext, CommandRouter, register_builtin_commands
from step58.consolidation import Consolidator
from step58.context import ContextBuilder, RequestContext
...
from step58.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step58.model_runtime import ModelRuntimeResolver
from step58.memory import MemoryStore
from step58.pairing import PairingStore
from step58.runtime_context import (
...
from step58.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step58.security.workspace_access import WorkspaceScopeResolver
from step58.session import Session, SessionManager
from step58.session import turn_continuation
from step58.session.history_visibility import HIDDEN_HISTORY_META
from step58.session.keys import UNIFIED_SESSION_KEY
from step58.subagent import SubagentManager
from step58.tool import ToolRegistry
from step58.context import ToolContext
from step58.loader import ToolLoader
from step58.helpers import image_placeholder_text, truncate_text
from step58.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step58 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step58：summary 改名为 pending_summary（对齐 nanobot）
    ...
    stop_reason: str = ""
    # step58：移除 error 字段（错误通过 final_content + stop_reason 传递）
    had_injections: bool = False
    # step58：usage 字典（tokens 信息）
    ...
    # step58：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step58：跟踪后台任务，shutdown 时 drain
        ...
        # step58：runtime 管理委托给 ModelRuntimeResolver
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step58.providers.factory import make_provider
    ...
    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step58：登记+完成自动移除）。
    ...
    def llm_runtime(self) -> LLMRuntime:
        """解析下一个 turn 使用的运行时（step58：refresh 为 no-op）。"""
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step58：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step58：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step58：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_compact(self, ctx: TurnContext) -> str:
        ...
        # step58：summary → pending_summary
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step58：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    def _prepare_message_media(
        ...
        """处理消息附件：非图片追加引用，图片路径保留（step58）。

        step58 简化版：总是调用 reference_non_image_attachments（不提取
    ...
    def _build_initial_messages(
        ...
        # step58：处理 media 附件（非图片追加引用，图片追加占位符）
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step58.session.manager import replay_max_messages_for_context
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        # step58：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step58：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step58：typed event 路径，带 stream_id
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step58：带 stream_id，resuming=False 时递增 segment。
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        # step58：不再重建 ctx.result，直接使用扁平字段
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        # step58：ctx.result → ctx.final_content
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # step58：tokens 从 ctx.usage 获取（_run_agent_loop 暂不填充，默认 0+0）
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step58：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step58：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step58：将 runtime_context 元数据设置到历史消息
