...
step58 为最小增量版：支持 model 切换、preset 选择、context window 调整；
provider 热刷新（refresh）和预设表解析留待 step60 配置层。
"""
...
class ModelRuntimeResolver:
    ...
    与 nanobot 的差异：
    - 暂无 model_presets 表 / provider_snapshot_loader（step60 补齐）
    ...
    def provider_signature(self) -> tuple[object, ...] | None:
        """当前 provider 快照签名（step58 透传 runtime.snapshot_signature）。"""
    ...
    def current(self, *, refresh: bool = False) -> LLMRuntime:
        ...
        Args:
            refresh: 是否刷新配置（step58 为 no-op，step60 补齐）。

        Returns:
            当前 LLMRuntime。
        """
        # step58：无 provider_snapshot_loader，refresh 暂不实现
        return self._runtime
    ...
    def select_preset(self, name: str | None) -> LLMRuntime:
        """选择命名预设为默认（step58 简化版：仅设置 model_preset 字段）。
