...
from step113.context import RequestContext, ToolContext
from step113.hook import AgentHook, AgentHookContext
...
class SubagentManager:
    ...
    step113 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step113：manager 自己持有的长命令会话管理器，跨子代理共享
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        if self._provider is None:
            return
        # step113：从 origin 重建请求上下文与 workspace 范围，注入 AgentRunSpec；
