...
from step48.autocompact import AutoCompact
from step48.bus import MessageBus
from step48.channel import BaseChannel
from step48.channels.cli import CliChannel
from step48.command import CommandContext, CommandRouter, normalize_command_text
from step48.consolidation import Consolidator, _consolidation_boundary
from step48.goal_state import *
from step48.helpers import estimate_message_tokens, estimate_prompt_tokens
from step48.llm import Runtime
from step48.memory import MemoryStore
from step48.context import ContextBuilder
from step48.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step48.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step48.llm import LLMResponse, ToolCallRequest
from step48.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step48.manager import ChannelManager
from step48.pairing import PAIRING_CODE_META_KEY, PairingStore
from step48.provider import LLMProvider
from step48.governance import ContextGovernanceConfig, ContextGovernor
from step48.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step48.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step48.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step48.session import Session, SessionManager
from step48.subagent import SubagentManager, SubagentStatus
from step48.tool import ToolRegistry, Tool
from step48.tools.long_task import CreateGoalTool, UpdateGoalTool
from step48.tools.echo import EchoTool
from step48.tools.spawn import SpawnTool
from step48.bus.events import StreamDeltaEvent
from step48.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step48.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step48.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step48.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step48.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step48.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step48.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step48.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step48.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step48.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step48.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step48：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step48：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step48.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step48.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step48.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step48.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step48.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step48.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step48.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step48.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step48.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step48.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step48.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step48.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step48.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step48：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step48.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step48.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep48StateTrace(unittest.IsolatedAsyncioTestCase):
    """step48：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step48.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step48.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep48SaveTurnEnhancements(unittest.TestCase):
    """step48：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step48.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep48CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step48：_state_command shortcut 持久化。"""
...
class Teststep48DropMalformedToolCalls(unittest.TestCase):
    """step48：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep48MalformedRetryMessages(unittest.TestCase):
    """step48：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep48MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step48：_request_malformed_fallback 降级无工具请求。"""
...
class Teststep48FinalizationRetryMessages(unittest.TestCase):
    """step48：_finalization_retry_messages 构造重试消息。"""
...
class Teststep48FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step48：空响应耗尽后发 finalization retry 集成测试。"""
...
class TestStep48ExtractReasoning(unittest.TestCase):
    """step48：extract_reasoning 剥离推理内容。"""

    def test_extract_reasoning_strips_think_tags(self):
        """<think> 标签从 content 中剥离，reasoning 被提取。"""
        from step48.helpers import extract_reasoning
...
class TestStep48ReasoningIntegration(unittest.IsolatedAsyncioTestCase):
    """step48：reasoning 提取 + emit_reasoning 集成测试。"""
...
class TestStep48FinalizeContent(unittest.IsolatedAsyncioTestCase):
    """step48：hook.finalize_content 被调用。"""
