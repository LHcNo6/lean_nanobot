...
from step48.hook import AgentHook, AgentHookContext, AgentRunHookContext
from step48.llm import LLMResponse
from step48.provider import LLMProvider
from step48.tool import Tool, ToolRegistry
...
class AgentRunner:
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        from step48.security.workspace_access import bind_workspace_scope, reset_workspace_scope
        from step48.tools.file_state import FileStates, bind_file_states, reset_file_states
    ...
    async def _request_model(
        ...
        *,
        ...
        # step48：malformed tool_call 处理——递归重试 + 降级无工具
    ...
    def _drop_malformed_tool_calls(
        ...
        """step48：返回 (dropped_count, all_dropped, original_finish_reason)。
    ...
    def _malformed_tool_call_retry_messages(
        ...
        """step48：构造 malformed tool_call 重试提示消息（保留原 assistant 文本）。"""
    ...
    async def _request_malformed_fallback(
        ...
        """step48：malformed_retry 仍失败时降级为无工具请求（直接调 provider）。"""
    ...
    def _finalization_retry_messages(
        ...
        """step48：构造 finalization 重试消息（无工具，让模型基于对话生成最终答案）。"""
        retry_messages = list(messages)
    ...
    def _append_injected_messages(
        ...
        from step48.session.history_visibility import is_hidden_history_message
    ...
    async def _run_loop(
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.tool_calls and response.finish_reason == "tool_calls":
                # step48：malformed 已在 _request_model 中处理，直接用 response
            ...
            if self._is_blank_text(clean):
                ...
                # step48：空响应重试耗尽后发 finalization retry（无工具请求），
