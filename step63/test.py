...
from step63.autocompact import AutoCompact
from step63.bus import MessageBus
from step63.channel import BaseChannel
from step63.channels.cli import CliChannel
from step63.command import CommandContext, CommandRouter, normalize_command_text
from step63.consolidation import Consolidator, _consolidation_boundary
from step63.goal_state import *
from step63.helpers import estimate_message_tokens, estimate_prompt_tokens
from step63.llm import Runtime
from step63.memory import MemoryStore
from step63.context import ContextBuilder
from step63.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step63.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step63.llm import LLMResponse, ToolCallRequest
from step63.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step63.manager import ChannelManager
from step63.pairing import PAIRING_CODE_META_KEY, PairingStore
from step63.provider import LLMProvider
from step63.governance import ContextGovernanceConfig, ContextGovernor
from step63.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step63.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step63.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step63.session import Session, SessionManager
from step63.subagent import SubagentManager, SubagentStatus
from step63.tool import ToolRegistry, Tool
from step63.tools.long_task import CreateGoalTool, UpdateGoalTool
from step63.tools.echo import EchoTool
from step63.tools.spawn import SpawnTool
from step63.bus.events import StreamDeltaEvent
from step63.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step63.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step63.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step63.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step63.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step63.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step63.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step63.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step63.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step63.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step63.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step63：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step63：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step63.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step63.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step63.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step63.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step63.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step63.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step63.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step63.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step63.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step63.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step63.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step63.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step63.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step63：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step63.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step63.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep63StateTrace(unittest.IsolatedAsyncioTestCase):
    """step63：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step63.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step63.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep63SaveTurnEnhancements(unittest.TestCase):
    """step63：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step63.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep63CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step63：_state_command shortcut 持久化。"""
...
class Teststep63DropMalformedToolCalls(unittest.TestCase):
    """step63：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep63MalformedRetryMessages(unittest.TestCase):
    """step63：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep63MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step63：_request_malformed_fallback 降级无工具请求。"""
...
class Teststep63FinalizationRetryMessages(unittest.TestCase):
    """step63：_finalization_retry_messages 构造重试消息。"""
...
class Teststep63FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step63：空响应耗尽后发 finalization retry 集成测试。"""
...
class Teststep63ExtractReasoning(unittest.TestCase):
    """step63：extract_reasoning 剥离推理内容。"""

    def test_extract_reasoning_strips_think_tags(self):
        """<think> 标签从 content 中剥离，reasoning 被提取。"""
        from step63.helpers import extract_reasoning
...
class Teststep63ReasoningIntegration(unittest.IsolatedAsyncioTestCase):
    """step63：reasoning 提取 + emit_reasoning 集成测试。"""
...
class Teststep63FinalizeContent(unittest.IsolatedAsyncioTestCase):
    """step63：hook.finalize_content 被调用。"""
...
class Teststep63UsageTools(unittest.TestCase):
    """step63：usage 工具方法（_usage_dict / _usage_total / _merge_usage）。"""

    def test_usage_dict_converts_values(self):
        """_usage_dict 正确转换 usage 字典为 int，过滤非数字值。"""
        from step63.runner import AgentRunner
...
class Teststep63UsageEstimate(unittest.TestCase):
    """step63：_estimate_response_usage / _usage_or_estimate。"""
    ...
    def test_estimate_response_usage_returns_total_and_estimated(self):
        ...
        from step63.runner import AgentRunner, AgentRunSpec, LLMResponse
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hello"}],
            tools=_MockToolRegistry(),
            provider=_MockProvider(),
...
class Teststep63AccumulateUsage(unittest.TestCase):
    """step63：_accumulate_usage 累计所有键。"""

    def test_accumulate_usage_counts_all_keys(self):
        """_accumulate_usage 累计所有键（包括 total_tokens/estimated_tokens）。"""
        from step63.runner import AgentRunner, LLMResponse
...
class _step63CancelledRegistry:
...
class _step63TrackingHook(AgentHook):
    """记录工具执行 hook 调用。"""
    def __init__(self):
        self.before_calls = []
        self.after_calls = []
...
class Teststep63RunTool(unittest.IsolatedAsyncioTestCase):
    """step63：_run_tool 三元组返回 + hook 生命周期。"""

    def _make_ctx(self):
        from step63.hook import AgentHookContext
        return AgentHookContext(iteration=0, messages=[])

    def _make_spec(self, registry):
        from step63.runner import AgentRunSpec
        return AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
            tools=registry,
            provider=_MockProvider(),
    ...
    async def test_run_tool_returns_triple(self):
        ...
        from step63.runner import AgentRunner
        from step63.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step63EchoRegistry()
        spec = self._make_spec(registry)
        hook = _step63TrackingHook()
        ctx = self._make_ctx()
    ...
    async def test_run_tool_calls_on_error_hook(self):
        ...
        self.assertIsNone(error)  # step63 不设 fatal_error（留到 step63 fail_on_tool_error）
    ...
    async def test_run_tool_cancelled_error_propagates(self):
        ...
        from step63.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step63CancelledRegistry()
        spec = self._make_spec(registry)
        hook = _step63TrackingHook()
        ctx = self._make_ctx()
        with self.assertRaises(asyncio.CancelledError):
...
class Teststep63ExecuteToolBatch(unittest.IsolatedAsyncioTestCase):
    """step63：_execute_tool_batch 三元组返回。"""

    async def test_execute_tool_batch_returns_triple(self):
        """_execute_tool_batch 返回 (results, events, fatal_error)。"""
        from step63.runner import AgentRunner, AgentRunSpec
        from step63.governance import ContextGovernanceConfig
        from step63.hook import AgentHookContext, AgentHook
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _step63EchoRegistry("ok")
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
...
class Teststep63RunCancelledError(unittest.IsolatedAsyncioTestCase):
    """step63：run() 中 CancelledError 分离。"""

    async def test_run_cancelled_error_no_on_error(self):
        """run() 中 CancelledError 不调 on_error。"""
        from step63.runner import AgentRunner, AgentRunSpec
        from step63.hook import AgentHook
...
class Teststep63SSRFDetection(unittest.TestCase):
    """step63：SSRF 检测方法。"""
...
class Teststep63ClassifyViolation(unittest.TestCase):
    """step63：_classify_violation 统一分类。"""
...
class Teststep63RepeatedLookup(unittest.TestCase):
    """step63：重复外部查找和 workspace 违规检测。"""

    def test_repeated_external_lookup_blocks_after_2(self):
        """重复外部查找 2 次后阻断。"""
        from step63.helpers import repeated_external_lookup_error
        counts = {}
        # 前 2 次不阻断
    ...
    def test_repeated_workspace_violation_escalates_after_2(self):
        ...
        from step63.helpers import repeated_workspace_violation_error
        counts = {}
        # 前 2 次不升级
...
class Teststep63RunToolSecurity(unittest.IsolatedAsyncioTestCase):
    """step63：_run_tool 中安全检测集成。"""
...
class TestStep52SpecFields(unittest.TestCase):
    ...
    def test_tool_events_default_empty(self):
        ...
        from step63.runner import AgentRunResult
        result = AgentRunResult(final_content="hi", messages=[])
        self.assertEqual(result.tool_events, [])


class Teststep63FailOnToolError(unittest.IsolatedAsyncioTestCase):
    """step63：fail_on_tool_error 终止 turn。"""
...
class Teststep63ToolEvents(unittest.IsolatedAsyncioTestCase):
    """step63：tool_events 收集。"""
...
class Teststep63SpecFields(unittest.TestCase):
    """step63：AgentRunSpec 新字段。"""
...
class Teststep63IncrementalThinkExtractor(unittest.IsolatedAsyncioTestCase):
    """step63：IncrementalThinkExtractor 基本功能。"""

    async def test_feed_extracts_think(self):
        """feed 提取 <think> 块中的新文本并 emit。"""
        from step63.helpers import IncrementalThinkExtractor
        emitted = []
        async def emit(text):
            emitted.append(text)
...
class Teststep63StreamingThinking(unittest.IsolatedAsyncioTestCase):
    """step63：流式 thinking 提取。"""
...
class TestStep54StreamSegmentation(unittest.IsolatedAsyncioTestCase):
    ...
    def test_stream_delta_typed_event_has_stream_id(self):
        ...
        from step63.bus.outbound_events import StreamDeltaEvent as TypedDelta
...
class TestStep55ModelRuntimeResolver(unittest.TestCase):
    """step63：ModelRuntimeResolver 核心功能。"""

    def _make_runtime(self, model="gpt-4", preset=None, sig=None):
        from step63.llm import GenerationSettings, LLMRuntime
        return LLMRuntime(
            provider=object(),
            model=model,
    ...
    def test_runtime_property_returns_initial(self):
        from step63.model_runtime import ModelRuntimeResolver
...
class TestStep55AgentLoopRuntime(unittest.TestCase):
    """step63：AgentLoop runtime 委托给 ModelRuntimeResolver。"""
...
class TestStep56DocumentUtils(unittest.TestCase):
    """step63：utils/document.py 附件处理。"""

    def test_is_image_file_png(self):
        from step63.utils.document import is_image_file
    ...
    def test_reference_non_image_attachments_separates(self):
        from step63.utils.document import reference_non_image_attachments
...
class TestStep56ImagePlaceholder(unittest.TestCase):
    """step63：image_placeholder_text。"""

    def test_with_path(self):
        from step63.helpers import image_placeholder_text
...
class TestStep56PrepareMessageMedia(unittest.TestCase):
    """step63：AgentLoop._prepare_message_media。"""
...
class TestStep57TurnContextFields(unittest.TestCase):
    """step63：TurnContext 移除 result/error/summary，改用扁平字段。"""

    def test_turn_context_no_result_field(self):
        """TurnContext 不再有 result 字段。"""
        from step63.loop import TurnContext
        ctx = TurnContext(
            msg=InboundMessage(content="hi"),
            session_key="test",
        )
...
class TestStep58ModelErrorPlaceholder(unittest.TestCase):
    """step63：_append_model_error_placeholder。"""

    def test_append_placeholder(self):
        from step63.runner import AgentRunner, _PERSISTED_MODEL_ERROR_PLACEHOLDER
        messages = [{"role": "user", "content": "hi"}]
    ...
    def test_placeholder_constant_text(self):
        from step63.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
        self.assertIn("model error", _PERSISTED_MODEL_ERROR_PLACEHOLDER.lower())


class TestStep58IsToolErrorResult(unittest.TestCase):
    """step63：is_tool_error_result。"""

    def test_error_result(self):
        from step63.tool import ToolResult, is_tool_error_result
    ...
    def test_non_tool_result(self):
        from step63.tool import is_tool_error_result
...
class TestStep59RequestContextForTurn(unittest.TestCase):
    """step63：_request_context_for_turn 重命名。"""
...
class TestStep59WorkspaceScopeForTurn(unittest.TestCase):
    """step63：_build_agent_spec 使用 for_turn。"""

    def test_for_turn_method_exists(self):
        """WorkspaceScopeResolver.for_turn 方法存在。"""
        from step63.security.workspace_access import WorkspaceScopeResolver
...
class TestStep59SystemMessageExtendToUser(unittest.TestCase):
    """step63：_process_system_message 中 extend_to_user=is_subagent。"""
...
class Teststep63ChannelsConfigExtractDocumentText(unittest.TestCase):
    """step63：ChannelsConfig.extract_document_text 字段。"""

    def test_default_true(self):
        from step63.config.schema import ChannelsConfig
        cfg = ChannelsConfig()
...
class Teststep63ToolsConfigWebExec(unittest.TestCase):
    """step63：ToolsConfig.web/exec 子配置。"""

    def test_web_default(self):
        from step63.config.schema import ToolsConfig
        cfg = ToolsConfig()
...
class Teststep63AgentLoopConfigParams(unittest.TestCase):
    """step63：AgentLoop 接收 channels_config/tools_config。"""
...
class Teststep63PrepareMessageMediaWithChannelsConfig(unittest.TestCase):
    """step63：_prepare_message_media 读取 channels_config。"""
