...
def _get_token_encoding() -> Any:
    """获取 tiktoken cl100k_base 编码器。

    调用方应在 try-except 中调用，tiktoken 未安装时会抛出 ImportError。
    """
    import tiktoken
...
def truncate_text_to_tokens(text: str, max_tokens: int) -> str:
    """按 token 预算截断文本，带稳定后缀。

    与 ``truncate_text`` 不同，本函数精确计算 token 数，对中文/CJK/代码
    等内容更准确。tiktoken 不可用时回退到 4 chars/token 估算。

    Args:
        text: 待截断文本。
        max_tokens: 最大 token 数（含后缀）。

    Returns:
        截断后的文本；max_tokens <= 0 或文本未超限时返回原文。
    """
    if max_tokens <= 0:
        return text
    try:
        ...
        # 从 body_budget 往下找最大的 candidate，确保总 token <= max_tokens
...
# ---- step64：SSRF/workspace 安全检测辅助函数 ----
...
def image_placeholder_text(path: str | None, *, empty: str = "[image]") -> str:
    """构建图片占位符文本（step64：对齐 nanobot helpers.image_placeholder_text）。

    Args:
        path: 图片路径，None 或空时返回 empty。
        empty: 无路径时的占位符，默认 "[image]"。
