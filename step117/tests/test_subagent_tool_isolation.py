...
from step117.config.schema import Config, ToolsConfig
...
from step117.security.workspace_access import default_workspace_scope
from step117.subagent import SubagentManager, _flatten_tools_config
...
class TestSubagentRequestContextBinding(unittest.IsolatedAsyncioTestCase):
    """step117：子代理运行期 RequestContext / workspace_scope 绑定（契约 B1-B3）。"""
...
class TestSubagentRuntimeLimitSync(unittest.IsolatedAsyncioTestCase):
    """step117：子代理 llm_timeout_s 同步父会话墙钟超时策略（契约 D1+D2）。"""
