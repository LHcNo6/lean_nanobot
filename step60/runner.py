...
from step60.hook import AgentHook, AgentHookContext, AgentRunHookContext
from step60.llm import LLMResponse
from step60.provider import LLMProvider
from step60.tool import Tool, ToolRegistry, ToolResult
...
# step60：模型错误时持久化到历史的占位符（对齐 nanobot）
...
class AgentRunner:
    ...
    def _merge_message_content(left: Any, right: Any) -> str | list[dict[str, Any]]:
        """合并两条消息的 content（step60：对齐 nanobot）。
    ...
    def _build_request_kwargs(
        ...
        *,
        ...
        """构建 LLM provider 请求参数（step60：从 _request_model 提取）。
    ...
    def _append_final_message(messages: list[dict[str, Any]], content: str | None) -> None:
        """追加最终 assistant 消息，避免重复（step60：对齐 nanobot）。
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        from step60.security.workspace_access import bind_workspace_scope, reset_workspace_scope
        from step60.tools.file_state import FileStates, bind_file_states, reset_file_states
        ...
        try:
            ...
            raise  # step60：CancelledError 不调 on_error
    ...
    async def _request_model(
        ...
        *,
        ...
        # step60：流式 thinking 提取状态（仅非流式 + progress_callback 时启用）
        stream_buf = ""
        think_extractor = (
            IncrementalThinkExtractor()
        ...
        # step60：使用 _build_request_kwargs 构建请求参数
        ...
        # step60：流式结束后如果 reasoning 仍开放，emit end
        if reasoning_open:
            await hook.emit_reasoning_end()
            reasoning_open = False
        ...
        # step60：malformed tool_call 处理——递归重试 + 降级无工具
    ...
    def _estimate_response_usage(
        ...
        """step60：provider 感知的链式估算，替代简单 chars//4。
    ...
    def _usage_or_estimate(
        ...
        """step60：优先用真实 usage，缺失时估算；error 响应返回空 dict。"""
    ...
    def _drop_malformed_tool_calls(
        ...
        """step60：返回 (dropped_count, all_dropped, original_finish_reason)。
    ...
    def _malformed_tool_call_retry_messages(
        ...
        """step60：构造 malformed tool_call 重试提示消息（保留原 assistant 文本）。"""
    ...
    async def _request_malformed_fallback(
        ...
        """step60：malformed_retry 仍失败时降级为无工具请求（直接调 provider）。"""
    ...
    def _finalization_retry_messages(
        ...
        """step60：构造 finalization 重试消息（无工具，让模型基于对话生成最终答案）。"""
        retry_messages = list(messages)
    ...
    async def _execute_tool_batch(
        ...
        """step60：返回 (results, events, fatal_error) 三元组。"""
    ...
    async def _run_tool(
        ...
        """step60：返回 (result, event, error) 三元组 + hook 生命周期 + 安全检测。"""
    ...
    def _build_goal_continue_message(self, spec: AgentRunSpec) -> dict[str, str]:
        # step60：支持 callable goal_continue_message（闭包动态读取 session.metadata）
    ...
    def _append_injected_messages(
        ...
        from step60.session.history_visibility import is_hidden_history_message
    ...
    async def _run_loop(
        ...
        # step60：SSRF/workspace 安全检测计数（整个 turn 中持久化）
        external_lookup_counts: dict[str, int] = {}
        workspace_violation_counts: dict[str, int] = {}
        # step60：工具执行事件追踪
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.tool_calls and response.finish_reason == "tool_calls":
                # step60：malformed 已在 _request_model 中处理，直接用 response
            ...
            # step60：提取 reasoning，分离 reasoning_content/thinking_blocks 和 content
            reasoning_text, cleaned_content = extract_reasoning(
                response.reasoning_content, response.thinking_blocks, response.content,
            )
            response.content = cleaned_content

            # step60：输出 reasoning 流（非流式，一次性输出；流式留到 step60）
            if reasoning_text and not iter_ctx.streamed_reasoning:
                await hook.emit_reasoning(reasoning_text)
                await hook.emit_reasoning_end()
                iter_ctx.streamed_reasoning = True

            # step60：用 hook.finalize_content 替代直接用 response.content
            ...
            if self._is_blank_text(clean):
                ...
                # step60：空响应重试耗尽后发 finalization retry（无工具请求），
                ...
                # step60：使用 _append_final_message 避免重复
    ...
    def _accumulate_usage(total: dict[str, int], response: LLMResponse) -> None:
        """step60：累计 response.usage 中的所有键（包括 total_tokens/estimated_tokens）。"""
        if response.usage:
            for key, value in response.usage.items():
                if value:
                    total[key] = total.get(key, 0) + value

    # ---- step60：SSRF/workspace 安全检测 ----
