...
from step60.bus.outbound_events import (
    RetryWaitEvent,
...
from step60.bus.progress import build_bus_progress_callback
from step60.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step60.autocompact import AutoCompact
from step60.command import CommandContext, CommandRouter, register_builtin_commands
from step60.consolidation import Consolidator
from step60.context import ContextBuilder, RequestContext
...
from step60.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step60.model_runtime import ModelRuntimeResolver
from step60.memory import MemoryStore
from step60.pairing import PairingStore
from step60.runtime_context import (
...
from step60.session import turn_continuation
from step60.session.history_visibility import HIDDEN_HISTORY_META
from step60.session.keys import UNIFIED_SESSION_KEY
from step60.subagent import SubagentManager
from step60.tool import ToolRegistry
from step60.context import ToolContext
from step60.loader import ToolLoader
from step60.helpers import image_placeholder_text, truncate_text
from step60.utils.cancellation import task_is_cancelling
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step60 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step60：summary 改名为 pending_summary（对齐 nanobot）
    ...
    stop_reason: str = ""
    # step60：移除 error 字段（错误通过 final_content + stop_reason 传递）
    had_injections: bool = False
    # step60：usage 字典（tokens 信息）
    ...
    # step60：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step60：跟踪后台任务，shutdown 时 drain
        ...
        # step60：通道/工具配置（对齐 nanobot，供 media 处理等消费）
        ...
        # step60：runtime 管理委托给 ModelRuntimeResolver
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step60.providers.factory import make_provider
        ...
        return cls(
            ...
            # step60：传递 channels_config 和 tools_config（对齐 nanobot）
    ...
    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step60：登记+完成自动移除）。
    ...
    def llm_runtime(self) -> LLMRuntime:
        """解析下一个 turn 使用的运行时（step60：refresh 为 no-op）。"""
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step60：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step60：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step60：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_compact(self, ctx: TurnContext) -> str:
        ...
        # step60：summary → pending_summary
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step60：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    def _prepare_message_media(
        ...
        """处理消息附件：非图片追加引用，图片路径保留（step56/step60）。

        step60：channels_config.extract_document_text 控制是否提取文档文本
        ...
        Returns:
            (更新后的 content, 图片路径列表)
        """
        # step60：extract_document_text 开关已贯通（当前不实现提取逻辑）
    ...
    def _build_initial_messages(
        ...
        # step60：处理 media 附件（非图片追加引用，图片追加占位符）
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step60.session.manager import replay_max_messages_for_context
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        # step60：改用 for_turn（对齐 nanobot），支持 scoped channel 解析
        ...
        # step60：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step60：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step60：typed event 路径，带 stream_id
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step60：带 stream_id，resuming=False 时递增 segment。
    ...
    async def _process_system_message(
        ...
        *,
        ...
        history = session.get_history(
            ...
            # step60：subagent follow-up 时 extend_to_user=True（对齐 nanobot）
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        # step60：不再重建 ctx.result，直接使用扁平字段
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        # step60：ctx.result → ctx.final_content
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # step60：tokens 从 ctx.usage 获取（_run_agent_loop 暂不填充，默认 0+0）
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step60：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step60：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step60：将 runtime_context 元数据设置到历史消息
    ...
    async def run_dream(self, tools: ToolRegistry | None = None) -> AgentRunResult | None:
        ...
        .. deprecated::
            将在 harness 阶段（step60）迁移到 ``process_direct(ephemeral=True)``。
            当前保留用于向后兼容。
        """
