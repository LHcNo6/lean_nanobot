...
class ModelPresetConfig(Base):
    ...
    def to_generation_settings(self) -> Any:
        from step60.llm import GenerationSettings
...
class ChannelsConfig(Base):
    ...
    # step60：是否从文档附件提取文本（对齐 nanobot，当前 lean 版仅做开关不实现提取）
...
class ProvidersConfig(Base):
    ...
    def convert_extra_providers(self) -> "ProvidersConfig":
        ...
        from step60.providers.registry import find_by_name
...
class WebToolsConfig(Base):
    """Web 工具配置（step60：对齐 nanobot 最小形态）。"""
...
class ExecToolConfig(Base):
    """Shell exec 工具配置（step60：对齐 nanobot 最小形态）。"""
...
class ToolsConfig(Base):
    ...
    Attributes:
        web: Web 工具配置（step60 新增）。
        exec: Shell exec 工具配置（step60 新增）。
...
class Config(Base):
    ...
    def _match_provider(
        ...
        from step60.providers.registry import PROVIDERS, find_by_name, find_by_model
