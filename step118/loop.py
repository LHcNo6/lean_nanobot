...
from step118.bus import MessageBus
from step118.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step118.bus.outbound_events import (
    RetryWaitEvent,
...
from step118.bus.progress import build_bus_progress_callback
from step118.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
)
from step118.autocompact import AutoCompact
from step118.command import CommandContext, CommandRouter, register_builtin_commands
from step118.consolidation import Consolidator
from step118.context import ContextBuilder, RequestContext
from step118.goal_state import (
    goal_state_runtime_lines,
    runner_wall_llm_timeout_s,
    sustained_goal_active,
)
from step118.hook import (
    AgentHook,
    AgentHookContext,
...
from step118.llm import LLMResponse, LLMRuntime, ToolCallRequest
from step118.model_runtime import ModelRuntimeResolver
from step118.memory import MemoryStore
from step118.pairing import PairingStore
from step118.runtime_context import (
...
from step118.runner import (
    AgentRunResult,
    AgentRunSpec,
    AgentRunner,
    _MAX_INJECTIONS_PER_TURN,
)
from step118.security.workspace_access import WorkspaceScopeResolver
from step118.session import Session, SessionManager
from step118.session import turn_continuation
from step118.session.history_visibility import HIDDEN_HISTORY_META
from step118.session.keys import UNIFIED_SESSION_KEY
from step118.subagent import SubagentManager
from step118.tool import ToolRegistry
from step118.context import ToolContext
from step118.loader import ToolLoader
from step118.tools.file_state import FileStateStore
from step118.tools.exec_session import ExecSessionManager
from step118.tools.cli_apps import build_cli_app_manager
from step118.helpers import image_placeholder_text, truncate_text
from step118.utils.cancellation import task_is_cancelling
from step118.utils.document import reference_non_image_attachments
...
class StateTraceEntry:
    """单个状态执行的追踪记录（step64 新增，对齐 nanobot）。
...
class TurnContext:
    ...
    # step64：summary 改名为 pending_summary（对齐 nanobot）
    ...
    stop_reason: str = ""
    # step64：移除 error 字段（错误通过 final_content + stop_reason 传递）
    had_injections: bool = False
    # step64：usage 字典（tokens 信息）
    ...
    # step64：状态执行追踪列表，每个状态执行后 append 一条 StateTraceEntry
    trace: list[StateTraceEntry] = field(default_factory=list)
    # step64：补齐 turn 级配置字段（对齐 nanobot）。
    # hooks：turn 级静态 hook，在全局 registered_hooks 之后执行；
    # hook_factories：turn 级 hook 工厂，在全局 registered_hook_factories 之后执行；
    # turn_scopes：turn 级 context manager，运行期间进入，结束后退出；
    # tools：turn 级工具覆盖，None 时使用默认 self.registry。
...
class AgentLoop:
    ...
    def __init__(
        ...
        # step64：跟踪后台任务，shutdown 时 drain
        ...
        # step64：通道/工具配置（对齐 nanobot，供 media 处理等消费）
        self.channels_config = channels_config
        self.tools_config = tools_config
        self._runner = AgentRunner()
        # step65：跨会话共享的文件状态查找表，按 session_key 管理 FileStates
        self._file_state_store = FileStateStore()
        self._exec_session_manager = ExecSessionManager()  # step73：长运行命令会话管理器
        # step118：CLI 应用白名单管理器（从 config.cli_apps.apps 注册，缺省空）
        ...
        # step64：runtime 管理委托给 ModelRuntimeResolver
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from step118.providers.factory import make_provider
        ...
        return cls(
            ...
            # step64：传递 channels_config 和 tools_config（对齐 nanobot）
            channels_config=extra.pop("channels_config", None) or config.channels,
            tools_config=extra.pop("tools_config", None) or config.tools,
            **extra,
        )

    def _schedule_background(self, coro: Any) -> None:
        """调度后台协程为被跟踪的 asyncio.Task（step64：登记+完成自动移除）。
    ...
    def llm_runtime(self) -> LLMRuntime:
        """解析下一个 turn 使用的运行时（step64：refresh 为 no-op）。"""
    ...
    async def close_mcp(self) -> None:
        """等待所有后台任务完成（step64：background tasks drain）。
    ...
    async def _process_message(
        ...
        *,
        ...
        while ctx.state != TurnState.DONE:
            ...
            # step64：记录状态开始时间（perf_counter 高精度计时）
            t0 = time.perf_counter()
            try:
                event = await handler(ctx)
            except Exception as exc:
                # step64：异常时记录 trace（error="exception"），保持现有 break 逻辑
    ...
    async def _state_compact(self, ctx: TurnContext) -> str:
        ...
        # step64：summary → pending_summary
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        # step64：shortcut 命令持久化 user+assistant（_command 标记）
        # /new 排除（它会清空会话）
        if raw.lower() != "/new":
    ...
    def _prepare_message_media(
        ...
        """处理消息附件：非图片追加引用，图片路径保留（step56/step64）。

        step64：channels_config.extract_document_text 控制是否提取文档文本
        （当前 lean 版仅做引用追加，不实现文本提取；开关已贯通）。
        ...
        Returns:
            (更新后的 content, 图片路径列表)
        """
        # step64：extract_document_text 开关已贯通（当前不实现提取逻辑）
        _channels_cfg = getattr(self, "channels_config", None)
        _extract = (
            _channels_cfg.extract_document_text
    ...
    def _build_initial_messages(
        ...
        # step64：处理 media 附件（非图片追加引用，图片追加占位符）
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        from step118.session.manager import replay_max_messages_for_context
        ...
        from step118.tools.message import MessageTool
        ...
        ctx.request_context = self._request_context_for_turn(
    ...
    def _build_agent_spec(
        ...
        *,
        ...
        step64：将 ``msg: InboundMessage`` 拆分为 ``channel``/``chat_id``/
        ``message_id``/``metadata``/``original_user_text`` 独立参数，对齐 nanobot
        ``_run_agent_loop`` 签名。

        Args:
            channel: 消息通道名（如 "cli"/"telegram"/"feishu"）。
            chat_id: 聊天 ID。
            session_key: 会话 key。
            session: 当前会话；None 表示无状态运行。
            initial_messages: 初始消息列表。
            message_id: 消息 ID（从 metadata 提取，用于 hook 上下文）。
            metadata: 消息元数据字典。
            original_user_text: 原始用户文本（用于可观测性/续跑判定）。
            request_context: 本 turn 的富 RequestContext（runner 绑定后工具可查）。
            workspace_scope: 本 turn 的 WorkspaceScope（runner 绑定后工具可查）。
            pending_budget_available: 本 turn 是否持有可排班续跑的 pending queue。
            hooks: turn 级静态 hook（仅本 turn 有效，在 registered_hooks 之后执行）。
        ...
        # step64：改用 for_turn（对齐 nanobot），支持 scoped channel 解析
        ...
        # step64：闭包动态读取 session.metadata 中的目标状态行
        ...
        # step64：_wants_stream 时启用 stream_id 分段（对齐 nanobot）。
        _wants_stream = bool((metadata or {}).get("_wants_stream"))
        ...
        async def _publish_delta(text: str) -> None:
            if not text:
                return
            if _wants_stream:
                # step64：typed event 路径，带 stream_id
                ...
                await self.bus.publish_outbound(StreamDeltaEvent(
                    content=text, channel=channel, chat_id=chat_id,
        ...
        async def _publish_stream_end(*, resuming: bool = False, **_: Any) -> None:
            ...
            # step64：带 stream_id，resuming=False 时递增 segment。
        ...
        hook = build_agent_turn_hook(AgentTurnHookSpec(
            ...
            # step61：turn 级静态 hook，在 registered_hooks 之后执行
        ...
        return AgentRunSpec(
            ...
            # step64：turn 级工具覆盖；None 时使用默认 self.registry
            tools=tools if tools is not None else self.registry,
    ...
    async def _process_system_message(
        ...
        *,
        ...
        history = session.get_history(
            ...
            # step64：subagent follow-up 时 extend_to_user=True（对齐 nanobot）
            extend_to_user=is_subagent,
        )
        # step29: 系统通道同样解析运行时上下文（provider 可依据
        # RequestContext.session_key 区分轮次）；assistant 角色不附加。
        request_context = self._request_context_for_turn(
    ...
    async def _run_agent_loop(
        ...
        *,
        ...
        step64：新增 ``hooks``（turn 级静态 hook）和 ``tools``（turn 级工具覆盖），
        对齐 nanobot ``TurnContext.hooks`` / ``TurnContext.tools``。
        ...
        Args:
            ...
            hooks: turn 级静态 hook（仅本 turn 有效）。step64 新增。
            hook_factories: turn 级 hook 工厂（仅本 turn 有效）。
            turn_scopes: turn 级 context manager 列表，运行期间进入，结束后退出。
            tools: turn 级工具覆盖；None 时使用默认 registry。step64 新增。
            ephemeral: 是否临时 turn（True 时 hook 链仅保留 progress hook）。
            run_extra_hooks_for_ephemeral: 临时 turn 是否也跑额外 hook。
        ...
        try:
            ...
            spec = self._build_agent_spec(
                ...
                injection_callback=self._build_injection_callback(
                    pending_queue, session_key, session,  # type: ignore[arg-type]
                ),
            ...
            # step64：保存本次 turn 的 hook 到实例属性，供 _state_run 读取，
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        final_content, tools_used, all_messages, stop_reason, had_injections = (
            await self._run_agent_loop(
                ...
                message_id=(ctx.msg.metadata or {}).get("message_id") if isinstance(ctx.msg.metadata, dict) else None,
                metadata=ctx.msg.metadata if isinstance(ctx.msg.metadata, dict) else None,
                original_user_text=ctx.msg.content if isinstance(ctx.msg.content, str) else None,
                session=ctx.session,
                session_key=session_key,
        ...
        # step64：不再重建 ctx.result，直接使用扁平字段
        ...
        # step64：从 _run_agent_loop 保存的 hook 读取，不再重建 _stream_spec。
        # 消除了重复调用 _build_agent_spec 的技术债（原注释承认"ToolLoader.load
        # 对已注册工具幂等"，但仍是不必要的重复构建）。
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        # step64：ctx.result → ctx.final_content
    ...
    async def _state_respond(self, ctx: TurnContext) -> str:
        ...
        # step64：tokens 从 ctx.usage 获取（_run_agent_loop 暂不填充，默认 0+0）
        ...
        *,
        ...
        for block in content:
            if not isinstance(block, dict):
                ...
                continue
            # step64：image_url data: 块替换为占位文本（避免 base64 写入历史）
            if (
    ...
    def _save_turn(
        ...
        *,
        ...
        for m in messages[skip:]:
            entry = dict(m)
            # step64：弹出内部 _meta，提取 runtime_context 元数据
            ...
            if role == "tool":
                ...
                if isinstance(content, list):
                    ...
                    if not filtered:
                        continue
                    entry["content"] = filtered
                # step64：将 runtime_context 元数据设置到历史消息
    ...
    async def run_dream(self, tools: ToolRegistry | None = None) -> AgentRunResult | None:
        """[DEPRECATED step64] 后台记忆整理（已迁移到 harness ``main.run_dream``）。

        .. deprecated::
            step64 起 dream 功能迁移到 harness 层（``main.run_dream(loop, tools)``）。
            loop 层保留此方法仅用于向后兼容（``command/builtin.py`` 的 /dream 命令
            仍在 loop 层调用）。新代码应使用 ``main.run_dream``。

        实现逻辑与 ``main.run_dream`` 完全一致：构建 dream prompt，直接调用
        ``runner.run``，不经过 ``_process_message`` 状态机。
        """
