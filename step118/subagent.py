...
from step118.bus.events import InboundMessage
from step118.config.schema import ToolsConfig
from step118.context import RequestContext, ToolContext
from step118.hook import AgentHook, AgentHookContext
...
from step118.skills.loader import SkillsLoader
...
class SubagentManager:
    ...
    step118 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step118：manager 自己持有的长命令会话管理器，跨子代理共享
        ...
        # step118：从原始 config 提取禁用技能，构造子代理技能加载器
        ...
        # step118：父会话墙钟超时解析回调（对齐 nanobot llm_wall_timeout_for_session）；
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        if self._provider is None:
            return
        # step118：从 origin 取出 workspace_scope，优先用以渲染 prompt 的工作区根；
        ...
        # step118：从 origin 重建请求上下文与 workspace 范围，注入 AgentRunSpec；
        ...
        # step118：以父会话 session_key 解析墙钟超时，写入 AgentRunSpec；
