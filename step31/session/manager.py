...
class Session:
    ...
    def get_history(
        ...
        from step31.consolidation import estimate_message_tokens
    ...
    def get_public_history(
        ...
        1. 先按 ``get_history`` 取未归档消息；
        2. 过滤 ``_hidden_history`` 标记的消息（如 subagent 内部注入）；
        3. 对剩余消息逐条调用 ``public_history_message`` 移除运行时上下文后缀。

        注意：与 ``get_history`` 不同，本方法返回的是**深拷贝**，不会影响
        session 内部存储。
        """
        from step31.runtime_context import public_history_message
        from step31.session.history_visibility import is_hidden_history_message

        history = self.get_history(max_messages=max_messages, max_tokens=max_tokens)
        return [
            public_history_message(msg)
            for msg in history
