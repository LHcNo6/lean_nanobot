...
from step31.helpers import (
    estimate_message_tokens,
    find_legal_message_start,
    truncate_text,
)
from step31.llm import Runtime
from step31.memory import MemoryStore, _ARCHIVE_SUMMARY_MAX_CHARS, _RAW_ARCHIVE_MAX_CHARS
...
class Consolidator:
    ...
    async def archive(self, messages: list[dict[str, Any]], *, runtime: Runtime, session_key: str | None = None, summary_messages: list[dict[str, Any]] | None = None) -> str | None:
        if not messages:
            return None
        # step31 (A12 下半场)：摘要前先移除运行时上下文后缀，避免
        # [Runtime Context — metadata only...] 等内部标记污染摘要内容。
        from step31.runtime_context import public_history_messages
