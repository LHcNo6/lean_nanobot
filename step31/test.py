...
from step31.autocompact import AutoCompact
from step31.bus import MessageBus
from step31.channel import BaseChannel
from step31.channels.cli import CliChannel
from step31.command import CommandContext, CommandRouter, normalize_command_text
from step31.consolidation import Consolidator, _consolidation_boundary
from step31.goal_state import *
from step31.helpers import estimate_message_tokens, estimate_prompt_tokens
from step31.llm import Runtime
from step31.memory import MemoryStore
from step31.context import ContextBuilder
from step31.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step31.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step31.llm import LLMResponse, ToolCallRequest
from step31.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step31.manager import ChannelManager
from step31.pairing import PAIRING_CODE_META_KEY, PairingStore
from step31.provider import LLMProvider
from step31.governance import ContextGovernanceConfig, ContextGovernor
from step31.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step31.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step31.runner import AgentRunSpec, AgentRunner
from step31.session import Session, SessionManager
from step31.subagent import SubagentManager, SubagentStatus
from step31.tool import ToolRegistry, Tool
from step31.tools.long_task import CreateGoalTool, UpdateGoalTool
from step31.tools.echo import EchoTool
from step31.tools.spawn import SpawnTool
from step31.bus.events import StreamDeltaEvent
from step31.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step31.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step31.tool import ToolResult
...
class TestStreamingHooks(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_stream_end_called(self):
        ...
        await AgentRunner().run(spec)
        # step31：单轮文本 = 2 次 on_stream_end——模型响应后(resuming=True)
    ...
    async def test_no_stream_when_tool_calls(self):
        ...
        # step31：每轮响应后各 1 次 flush(resuming=True) + 最终收尾
...
class TestAgentLoopStreaming(unittest.IsolatedAsyncioTestCase):
    async def _drain_until_outbound(self, bus: MessageBus) -> tuple[list[StreamDeltaEvent], OutboundMessage]:
        # step31：流收尾改走 typed ``StreamEndEvent``（resuming 语义），
    ...
    async def test_loop_streaming_end_to_end(self):
        ...
        # step31：内容增量走 StreamDeltaEvent，流收尾走 typed StreamEndEvent
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step31.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step31.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step31.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step31.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step31.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step31.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step31.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step31.context import reset_request_context
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step31.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step31.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step31.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step31.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step31.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step31.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step31.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step31.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step31.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step31.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step31.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step31.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step31.command.builtin import register_builtin_commands
