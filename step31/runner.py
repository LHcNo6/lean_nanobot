...
from step31.hook import AgentHook, AgentHookContext, AgentRunHookContext
from step31.llm import LLMResponse
from step31.provider import LLMProvider
from step31.tool import Tool, ToolRegistry
...
# step31（A8）：欠费/配额不足类错误（HTTP 402 或 billing 语义 token）不会被
...
class AgentRunSpec:
    ...
    Attributes:
        ...
        finalize_on_max_iterations: max_iterations 边界是否产出收尾响应。
            ...
            （step29 仅开关语义；step31 将补充"收尾 = 一次无工具 LLM 调用"。）
    """
    ...
    # step31（A8）：可定制文案——error 终止时用 ``error_message``（缺省回退
...
class AgentRunner:
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        from step31.security.workspace_access import bind_workspace_scope, reset_workspace_scope
    ...
    def _error_result(
        ...
        # step31（A8）：文案优先级——自定义 error_message > 欠费识别
    ...
    async def _request_model(
        ...
        # step31（A8）：usage 缺失时按文本长度估算（~4 字符/token），保证
    ...
    def _append_injected_messages(
        ...
        from step31.session.history_visibility import is_hidden_history_message
    ...
    async def _run_loop(
        ...
        for iteration in range(spec.max_iterations):
            ...
            # step31：对齐 nanobot——模型响应后先别关流（工具执行/重试/注入
            ...
            # step31（A8）：工具只在 ``should_execute_tools`` 下执行——
            ...
            # step31：对齐 nanobot——先查注入再决定关流：还有注入续跑时
        ...
        # step29：max_iterations 边界——finalize_on_max_iterations=False 时
        # 不产出合成 fallback 文案（隐形续跑接管），final_content 保持 None，
        # loop 的 maybe_continue_turn 会排班续跑片并剥掉尾条合成消息。
        # step31：边界处关流——可续跑时保持流存活（resuming=True），否则收尾。
        await hook.on_stream_end(iter_ctx, resuming=not spec.finalize_on_max_iterations)
        final_content = None
        if spec.finalize_on_max_iterations:
            # step31（A8）：收尾文案可定制（缺省 _MAX_ITERATIONS_FALLBACK）。
