...
from step114.bus.events import InboundMessage
from step114.config.schema import ToolsConfig
from step114.context import RequestContext, ToolContext
from step114.hook import AgentHook, AgentHookContext
...
class SubagentManager:
    ...
    step114 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step114：manager 自己持有的长命令会话管理器，跨子代理共享
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        if self._provider is None:
            return
        # step114：从 origin 重建请求上下文与 workspace 范围，注入 AgentRunSpec；
