...
from step125.bus.events import InboundMessage
from step125.config.schema import ToolsConfig
from step125.context import RequestContext, ToolContext
from step125.hook import AgentHook, AgentHookContext
...
from step125.security.workspace_access import workspace_sandbox_status
...
from step125.skills.loader import SkillsLoader


# ---------------------------------------------------------------------------
# 子代理 announce 模板渲染（step125，对齐 nanobot agent/subagent_announce.md）
# ---------------------------------------------------------------------------
...
def _render_subagent_announce(label: str, status_text: str, task: str, result: str) -> str:
    """渲染子代理 announce 内容（step125，对齐 nanobot 模板）。
...
def _extract_fail_on_tool_error(config: Any) -> bool:
    """从配置安全提取子代理是否将工具错误升级为失败（step125）。
...
class SubagentManager:
    ...
    step125 起不再复用主 agent 的工具注册表，而是通过 ``_build_tools()``
    ...
    def __init__(
        ...
        *,
        ...
        # step125：manager 自己持有的长命令会话管理器，跨子代理共享
        ...
        # step125：从原始 config 提取禁用技能，构造子代理技能加载器
        self._disabled_skills = _extract_disabled_skills(config)
        # step125：从原始 config 提取子代理运行配置（对齐 nanobot 子代理运行配置传播）
        ...
        # step125：父会话墙钟超时解析回调（对齐 nanobot llm_wall_timeout_for_session）；
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        tool_ctx = ToolContext(
            ...
            # step125（G9）：对齐 nanobot，子代理 ToolContext 携带宿主 workspace 限制状态
    ...
    async def spawn(
        ...
        *,
        ...
        # step125（G7）：temperature 覆写 —— 将覆写后的 runtime 写回 origin，
    ...
    async def _run_subagent(self, task_id: str, task: str, label: str, origin: dict, status: SubagentStatus) -> None:
        ...
        step125（G5，runtime 逐父同步）：从 ``origin["runtime"]`` 衍生
        ...
        # step125（G10）：用 checkpoint_callback 把 runner 迭代相位同步到 status.phase；
        ...
        try:
            result = await self.runner.run(AgentRunSpec(
                ...
                # step125（G10）：runner 每轮迭代回调，驱动多相位状态
    ...
    async def _announce(self, task_id: str, label: str, task: str, result: str, origin: dict, status: str) -> None:
        # step125：对齐 nanobot，用模板渲染 announce（而非内联 f-string）
        ...
        # step125：对齐 nanobot，透传 origin_message_id 到 announce 元数据
    ...
    def get_task_statuses(self) -> list[dict[str, Any]]:
        """返回所有子代理任务状态的快照（step125，对齐 nanobot self/my 可观测）。
