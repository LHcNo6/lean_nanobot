"""step125: 子代理 ToolContext 沙箱 + 多相位状态测试（G9 + G10）。
...
from step125.security.workspace_access import WorkspaceSandboxStatus
from step125.subagent import SubagentManager


class TestSubagentToolContextSandbox(unittest.IsolatedAsyncioTestCase):
    """step125（G9）：子代理 ToolContext 注入 workspace_sandbox。"""
    ...
    def test_build_tools_injects_workspace_sandbox(self) -> None:
        ...
        with mock.patch("step125.subagent.ToolContext", _SpyToolContext):
            mgr._build_tools()
...
class TestSubagentPhaseCheckpoint(unittest.IsolatedAsyncioTestCase):
    """step125（G10）：checkpoint_callback 驱动 status.phase 多相位。"""
