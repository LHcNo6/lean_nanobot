# step125 配套文档：spawn temperature 覆写（G7）
...
- `LLMRuntime`（step125/llm.py:27）是 `frozen` dataclass，**无** `with_generation_overrides`；
...
- 仅覆写 `temperature`（G7 范围）；`with_generation_overrides` 顺带支持 `max_tokens` 以对齐
  nanobot 方法签名，但 step125 不通过 spawn 暴露 max_tokens 覆写（留待将来按需扩展）。
