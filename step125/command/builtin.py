...
from step125.command.router import CommandContext, CommandRouter
from step125.bus.events import OutboundMessage
from step125.goal_state import goal_state_runtime_lines
...
async def _cmd_history(ctx: CommandContext) -> OutboundMessage:
    ...
    for i, m in enumerate(session.messages):
        ...
        # step125（G6 通道部分）：展示边界清洗子代理 announce 正文
        # （移除 Task: / Summarize 脚手架，仅留头 + 截断结果）；持久化全文不变。
