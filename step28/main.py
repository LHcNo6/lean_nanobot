...
from step28.llm import LLMRuntime
...
from step28.providers.factory import build_provider_snapshot
from step28.session import SessionManager
...
async def main():
    ...
    # step28 (A9 演示): 注册一个 loop 级运行时上下文提供器 —— 每个 turn 前
    # 动态解析当前时间并追加到用户消息尾部（对齐 nanobot register_runtime_context_provider）。
    async def _clock_runtime_context(request_context: Any):
        from step28.runtime_context import RuntimeContextBlock, wrap_runtime_context_lines
