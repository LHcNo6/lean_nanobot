...
from step28.runtime_context import RuntimeContextBlock, append_runtime_context
...
@dataclass
class ToolContext:
    """工具装配上下文（step28：拿到真实 workspace / config / 权限意图）。

    Attributes:
        config: 装配时的 Config 对象（step25 起由 loop 透传，可空）。
        workspace: 项目根目录（str）；step28 起为 effective scope 的
...
class ContextBuilder:
    ...
    def skills(self) -> Any:
        ...
        if self._skills is None:
            from step28.skills import SkillsLoader
    ...
    def build_system_prompt(
        ...
        """构建 system prompt（step28 起支持按 turn 覆盖引导文件根目录）。
    ...
    def build_messages(
        ...
        merged, _meta = append_runtime_context(current_message, blocks)
