...
from step50.autocompact import AutoCompact
from step50.bus import MessageBus
from step50.channel import BaseChannel
from step50.channels.cli import CliChannel
from step50.command import CommandContext, CommandRouter, normalize_command_text
from step50.consolidation import Consolidator, _consolidation_boundary
from step50.goal_state import *
from step50.helpers import estimate_message_tokens, estimate_prompt_tokens
from step50.llm import Runtime
from step50.memory import MemoryStore
from step50.context import ContextBuilder
from step50.bus.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step50.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step50.llm import LLMResponse, ToolCallRequest
from step50.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step50.manager import ChannelManager
from step50.pairing import PAIRING_CODE_META_KEY, PairingStore
from step50.provider import LLMProvider
from step50.governance import ContextGovernanceConfig, ContextGovernor
from step50.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step50.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step50.runner import AgentRunSpec, AgentRunner, _EMPTY_FINAL_RESPONSE_MESSAGE
from step50.session import Session, SessionManager
from step50.subagent import SubagentManager, SubagentStatus
from step50.tool import ToolRegistry, Tool
from step50.tools.long_task import CreateGoalTool, UpdateGoalTool
from step50.tools.echo import EchoTool
from step50.tools.spawn import SpawnTool
from step50.bus.events import StreamDeltaEvent
from step50.bus.outbound_events import StreamEndEvent
...
async def _consume_final_response(bus):
    ...
    """
    from step50.bus.outbound_events import StreamEndEvent, StreamedResponseEvent
...
from step50.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step50.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step50.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step50.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step50.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step50.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step50.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step50.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step50.context import reset_request_context
...
class _AlwaysMalformedProvider(LLMProvider):
    ...
    async def chat(self, messages, tools=None, model=None, temperature=0.7, max_tokens=4096):
        self.call_count += 1
        # step50：tools=None 时（降级无工具请求）返回正常响应
        if tools is None:
...
class TestMalformedToolCallRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_name_dropped_and_retried(self):
        ...
        # step50：malformed_retry 在 _request_model 内部递归，provider 被调用 2 次
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step50.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step50.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step50.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step50.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step50.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step50.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step50.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step50.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step50.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step50.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step50.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step50.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step50.command.builtin import register_builtin_commands
...
class TestLoopCommandState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_command_persists_shortcut_with_command_marker(self):
        """step50：shortcut 命令持久化 user+assistant，带 _command 标记。"""
        loop, _ = self._make_loop()
...
class TestEphemeralMode(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ephemeral_hook_only_progress(self):
        from step50.hook import AgentTurnHookSpec, build_agent_turn_hook, AgentProgressHook
...
class TestStep42AssembleOutbound(unittest.IsolatedAsyncioTestCase):
    ...
    def test_assemble_outbound_message_tool_suppression(self):
        from step50.tools.message import MessageTool
        loop, _ = self._make_loop()
...
class Teststep50StateTrace(unittest.IsolatedAsyncioTestCase):
    """step50：StateTraceEntry 状态追踪测试。"""
    ...
    def test_state_trace_entry_creation(self):
        from step50.loop import StateTraceEntry, TurnState
    ...
    async def test_process_message_trace_has_restore_state(self):
        ...
        from step50.loop import TurnState
        loop, _ = self._make_loop()
        captured_ctx = {}
...
class Teststep50SaveTurnEnhancements(unittest.TestCase):
    """step50：_save_turn _meta 弹出 + runtime_context + image_url + updated_at。"""
    ...
    def test_save_turn_runtime_context_meta(self):
        ...
        from step50.runtime_context import RUNTIME_CONTEXT_HISTORY_META, RUNTIME_CONTEXT_MESSAGE_META
        loop = self._make_loop()
...
class Teststep50CommandPersistence(unittest.IsolatedAsyncioTestCase):
    """step50：_state_command shortcut 持久化。"""
...
class Teststep50DropMalformedToolCalls(unittest.TestCase):
    """step50：_drop_malformed_tool_calls 返回元组 + mutate response。"""
...
class Teststep50MalformedRetryMessages(unittest.TestCase):
    """step50：_malformed_tool_call_retry_messages 构造重试提示。"""
...
class Teststep50MalformedFallback(unittest.IsolatedAsyncioTestCase):
    """step50：_request_malformed_fallback 降级无工具请求。"""
...
class Teststep50FinalizationRetryMessages(unittest.TestCase):
    """step50：_finalization_retry_messages 构造重试消息。"""
...
class Teststep50FinalizationRetryIntegration(unittest.IsolatedAsyncioTestCase):
    """step50：空响应耗尽后发 finalization retry 集成测试。"""
...
class Teststep50ExtractReasoning(unittest.TestCase):
    """step50：extract_reasoning 剥离推理内容。"""

    def test_extract_reasoning_strips_think_tags(self):
        """<think> 标签从 content 中剥离，reasoning 被提取。"""
        from step50.helpers import extract_reasoning
...
class Teststep50ReasoningIntegration(unittest.IsolatedAsyncioTestCase):
    """step50：reasoning 提取 + emit_reasoning 集成测试。"""
...
class Teststep50FinalizeContent(unittest.IsolatedAsyncioTestCase):
    """step50：hook.finalize_content 被调用。"""
...
class Teststep50UsageTools(unittest.TestCase):
    """step50：usage 工具方法（_usage_dict / _usage_total / _merge_usage）。"""

    def test_usage_dict_converts_values(self):
        """_usage_dict 正确转换 usage 字典为 int，过滤非数字值。"""
        from step50.runner import AgentRunner
...
class Teststep50UsageEstimate(unittest.TestCase):
    """step50：_estimate_response_usage / _usage_or_estimate。"""
    ...
    def test_estimate_response_usage_returns_total_and_estimated(self):
        ...
        from step50.runner import AgentRunner, AgentRunSpec, LLMResponse
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hello"}],
            tools=_MockToolRegistry(),
            provider=_MockProvider(),
...
class Teststep50AccumulateUsage(unittest.TestCase):
    """step50：_accumulate_usage 累计所有键。"""

    def test_accumulate_usage_counts_all_keys(self):
        """_accumulate_usage 累计所有键（包括 total_tokens/estimated_tokens）。"""
        from step50.runner import AgentRunner, LLMResponse
...
class _Step50CancelledRegistry:
...
class _Step50TrackingHook(AgentHook):
    """记录工具执行 hook 调用。"""
    def __init__(self):
        self.before_calls = []
        self.after_calls = []
...
class TestStep50RunTool(unittest.IsolatedAsyncioTestCase):
    """step50：_run_tool 三元组返回 + hook 生命周期。"""

    def _make_ctx(self):
        from step50.hook import AgentHookContext
        return AgentHookContext(iteration=0, messages=[])

    def _make_spec(self, registry):
        from step50.runner import AgentRunSpec
        return AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
            tools=registry,
            provider=_MockProvider(),
    ...
    async def test_run_tool_returns_triple(self):
        ...
        from step50.runner import AgentRunner
        from step50.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _Step50EchoRegistry()
        spec = self._make_spec(registry)
        hook = _Step50TrackingHook()
        ctx = self._make_ctx()
    ...
    async def test_run_tool_calls_on_error_hook(self):
        ...
        self.assertIsNone(error)  # step50 不设 fatal_error（留到 step52 fail_on_tool_error）
    ...
    async def test_run_tool_cancelled_error_propagates(self):
        ...
        from step50.governance import ContextGovernanceConfig
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _Step50CancelledRegistry()
        spec = self._make_spec(registry)
        hook = _Step50TrackingHook()
        ctx = self._make_ctx()
        with self.assertRaises(asyncio.CancelledError):
...
class TestStep50ExecuteToolBatch(unittest.IsolatedAsyncioTestCase):
    """step50：_execute_tool_batch 三元组返回。"""

    async def test_execute_tool_batch_returns_triple(self):
        """_execute_tool_batch 返回 (results, events, fatal_error)。"""
        from step50.runner import AgentRunner, AgentRunSpec
        from step50.governance import ContextGovernanceConfig
        from step50.hook import AgentHookContext, AgentHook
        tc = ToolCallRequest(id="c1", name="echo", arguments={"text": "hi"})
        registry = _Step50EchoRegistry("ok")
        spec = AgentRunSpec(
            initial_messages=[{"role": "user", "content": "hi"}],
...
class TestStep50RunCancelledError(unittest.IsolatedAsyncioTestCase):
    """step50：run() 中 CancelledError 分离。"""

    async def test_run_cancelled_error_no_on_error(self):
        """run() 中 CancelledError 不调 on_error。"""
        from step50.runner import AgentRunner, AgentRunSpec
        from step50.hook import AgentHook
