...
from step50.hook import AgentHook, AgentHookContext, AgentRunHookContext
from step50.llm import LLMResponse
from step50.provider import LLMProvider
from step50.tool import Tool, ToolRegistry
...
class AgentRunner:
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        from step50.security.workspace_access import bind_workspace_scope, reset_workspace_scope
        from step50.tools.file_state import FileStates, bind_file_states, reset_file_states
        ...
        try:
            ...
            raise  # step50：CancelledError 不调 on_error
    ...
    async def _request_model(
        ...
        *,
        ...
        # step50：malformed tool_call 处理——递归重试 + 降级无工具
    ...
    def _estimate_response_usage(
        ...
        """step50：provider 感知的链式估算，替代简单 chars//4。
    ...
    def _usage_or_estimate(
        ...
        """step50：优先用真实 usage，缺失时估算；error 响应返回空 dict。"""
    ...
    def _drop_malformed_tool_calls(
        ...
        """step50：返回 (dropped_count, all_dropped, original_finish_reason)。
    ...
    def _malformed_tool_call_retry_messages(
        ...
        """step50：构造 malformed tool_call 重试提示消息（保留原 assistant 文本）。"""
    ...
    async def _request_malformed_fallback(
        ...
        """step50：malformed_retry 仍失败时降级为无工具请求（直接调 provider）。"""
    ...
    def _finalization_retry_messages(
        ...
        """step50：构造 finalization 重试消息（无工具，让模型基于对话生成最终答案）。"""
        retry_messages = list(messages)
    ...
    async def _execute_tool_batch(
        ...
        """step50：返回 (results, events, fatal_error) 三元组。"""
    ...
    async def _run_tool(
        ...
        """step50：返回 (result, event, error) 三元组 + hook 生命周期 + CancelledError 分离。"""
    ...
    def _append_injected_messages(
        ...
        from step50.session.history_visibility import is_hidden_history_message
    ...
    async def _run_loop(
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.tool_calls and response.finish_reason == "tool_calls":
                # step50：malformed 已在 _request_model 中处理，直接用 response
                ...
                for batch in batches:
                    ...
                    # step50：events/fatal_error 暂不使用（留到 step52 tool_events）
            ...
            if self._is_blank_text(clean):
                ...
                # step50：空响应重试耗尽后发 finalization retry（无工具请求），
    ...
    def _accumulate_usage(total: dict[str, int], response: LLMResponse) -> None:
        """step50：累计 response.usage 中的所有键（包括 total_tokens/estimated_tokens）。"""
        if response.usage:
