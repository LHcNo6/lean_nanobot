...
from step17a.goal_state import GOAL_STATE_KEY, MAX_GOAL_OBJECTIVE_CHARS, parse_goal_state
from step17a.tool import Tool, ToolResult
...
class CreateGoalTool(Tool):
    def __init__(self, sessions: SessionManager | None = None, session_key: str | None = None):
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                "objective": {
                    "type": "string",
                    "description": "The sustained objective for this session",
                },
                "ui_summary": {
                    "type": "string",
                    "description": "Optional one-line display label",
                },
            },
            "required": ["objective"],
        }
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        sess = self._sessions.get_or_create(self._session_key)
        ...
        objective = (kwargs.get("objective") or "").strip()
        if not objective:
        ...
        blob = {
            "status": "active",
            "objective": objective,
            "ui_summary": (kwargs.get("ui_summary") or "").strip()[:120],
            "started_at": _iso_now(),
        }
        ...
        return {
            ...
            "properties": {
                "action": {
                    ...
                    "description": "How to update the active goal",
                },
                "recap": {
                    "type": "string",
                    "description": "Brief honest recap for the user",
                },
                "objective": {
                    "type": "string",
                    "description": "Replacement objective (required only for 'replace')",
                },
                "ui_summary": {
                    "type": "string",
                    "description": "Optional one-line display label for replacement",
                },
            },
            "required": ["action"],
        }
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if normalized == "replace":
            ...
            blob = {
                ...
                "recap": (kwargs.get("recap") or "").strip(),
            }
            _save_goal_state(sess, blob)
        ...
        blob = {**prior, "status": status_map[normalized], "ended_at": ended, "recap": (kwargs.get("recap") or "").strip()}
        _save_goal_state(sess, blob)
        tail = (kwargs.get("recap") or "").strip()
        if tail:
