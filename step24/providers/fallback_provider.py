...
from step24.llm import LLMResponse, RetryConfig
from step24.provider import _StreamGuard
...
class FallbackProvider:
    ...
    async def _try_chain(
        ...
        if self._primary_available():
            ...
            try:
                response = await getattr(self._primary, method)(**kwargs)
        ...
        for idx, preset in enumerate(self._fallback_presets):
            ...
            try:
                try:
                    response = await getattr(fallback_provider, method)(**kwargs)
                    logger.info(
