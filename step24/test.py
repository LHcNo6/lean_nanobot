...
from step24.channel import BaseChannel
from step24.channels.cli import CliChannel
from step24.command import CommandContext, CommandRouter, normalize_command_text
from step24.consolidation import Consolidator, _consolidation_boundary
from step24.goal_state import *
from step24.helpers import estimate_message_tokens, estimate_prompt_tokens
from step24.llm import Runtime
from step24.memory import MemoryStore
from step24.context import ContextBuilder
from step24.events import InboundMessage, OutboundMessage, StreamDeltaEvent
from step24.hook import AgentHook, AgentHookContext, AgentRunHookContext, CompositeHook
from step24.llm import LLMResponse, ToolCallRequest
from step24.loop import AgentLoop, StreamPublishingHook, TurnContext, TurnState
from step24.manager import ChannelManager
from step24.pairing import PAIRING_CODE_META_KEY, PairingStore
from step24.provider import LLMProvider
from step24.governance import ContextGovernanceConfig, ContextGovernor
from step24.llm import GenerationSettings, LLMRuntime, ModelPreset, resolve_preset
from step24.providers import (
    FallbackProvider,
    ProviderSettings,
...
from step24.runner import AgentRunSpec, AgentRunner
from step24.session import Session, SessionManager
from step24.subagent import SubagentManager, SubagentStatus
from step24.tool import ToolRegistry, Tool
from step24.tools.long_task import CreateGoalTool, UpdateGoalTool
from step24.tools.echo import EchoTool
from step24.tools.spawn import SpawnTool
from step24.events import StreamDeltaEvent
...
from step24.tool import ToolResult
...
class TestMidTurnInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_tool_passes_request_session_key(self):
        ...
        from step24.context import RequestContext, request_context
...
class TestGovernanceHelpers(unittest.TestCase):
    def test_estimate_message_tokens(self):
        from step24.helpers import estimate_message_tokens as emt
        tokens = emt({"role": "user", "content": "hello"})
        self.assertGreaterEqual(tokens, 4)

    def test_estimate_prompt_tokens_with_tools(self):
        from step24.helpers import estimate_prompt_tokens
    ...
    def test_find_legal_message_start(self):
        from step24.helpers import find_legal_message_start
        msgs = [
    ...
    def test_truncate_text(self):
        from step24.helpers import truncate_text
        result = truncate_text("hello world", 5)
        self.assertIn("truncated", result)
        self.assertLessEqual(len(result), 20)

    def test_ensure_nonempty_tool_result(self):
        from step24.helpers import ensure_nonempty_tool_result
...
class TestCreateGoalTool(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        from step24.context import RequestContext, bind_request_context
    ...
    def tearDown(self):
        from step24.context import reset_request_context
...
class TestCastParams(unittest.TestCase):
    def setUp(self):
        from step24.tool import tool_parameters
...
class TestToolCreate(unittest.TestCase):
    def test_tool_create_default(self):
        from step24.context import ToolContext
...
class TestToolContext(unittest.IsolatedAsyncioTestCase):
    async def test_request_context_bind(self):
        from step24.context import RequestContext, bind_request_context, reset_request_context, current_request_context
    ...
    async def test_request_context_session_key_helper(self):
        from step24.context import RequestContext, bind_request_context, reset_request_context, current_request_session_key
...
class TestToolLoader(unittest.IsolatedAsyncioTestCase):
    async def test_discover_finds_tools(self):
        from step24.loader import ToolLoader
...
class TestRegistryExecuteViaPrepareCall(unittest.IsolatedAsyncioTestCase):
    async def test_execute_with_prepare_call(self):
        from step24.context import RequestContext, bind_request_context, reset_request_context
        registry = ToolRegistry()
        registry.register(EchoTool())
        ctx = RequestContext()
        token = bind_request_context(ctx)
...
class TestStep18Integration(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_call_with_runner(self):
        from step24.context import RequestContext
...
class TestCliChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_publishes_and_exits(self):
        ...
        got = []
        with mock.patch("step24.channels.cli.ainput", side_effect=["hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
    ...
    async def test_exit_stops_immediately(self):
        ...
        with mock.patch("step24.channels.cli.ainput", side_effect=["/exit"]):
            await channel.start()
        self.assertTrue(bus.inbound.empty())
        self.assertFalse(channel.is_running)
    ...
    async def test_on_command_consumed(self):
        ...
        got = []
        with mock.patch("step24.channels.cli.ainput", side_effect=["/dream", "hello", "/exit"]):
            async def responder():
                msg = await bus.consume_inbound()
                got.append(msg)
...
class TestChannelManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_with_retry_success(self):
        ...
        with mock.patch("step24.manager._SEND_RETRY_DELAYS", (0.01, 0.02)):
...
class Teststep22Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_command_end_to_end(self):
        ...
        loop_task = asyncio.create_task(loop.run())
        with mock.patch("step24.channels.cli.ainput", side_effect=["/new", "hi", "/exit"]):
            await manager.start_all()
...
class TestBuiltinCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help(self):
        from step24.command.builtin import register_builtin_commands
