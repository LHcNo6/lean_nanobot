...
from step123.context import current_request_session_key
from step123.schema import BooleanSchema, IntegerSchema, StringSchema, tool_parameters_schema
from step123.tool import Tool, ToolResult, tool_parameters
...
class ExecSessionInfo:
    """执行会话摘要（step84，用于 list_exec_sessions）。"""
    ...
    returncode: int | None
    owner_session_key: str | None = None  # 归属会话 key（None=无归属，全员可见）
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            owner_session_key: 归属会话 key（由当前请求上下文决定，None=无归属）。
        """
...
class ExecSessionManager:
    ...
    async def start(
        ...
        *,
        ...
        async with self._lock:
            ...
            # 归属打标：复用 step113 注入子代理运行上下文的 session_key
    ...
    def get(self, session_id: str) -> _ExecSession | None:
        """获取会话（按当前请求会话做 owner 隔离）。

        Args:
            session_id: 会话 ID。

        Returns:
            会话实例；不存在，或不属于当前请求会话（owner 不匹配）时返回 None。

        隔离规则：
            - 会话 owner 为 None（无归属）→ 对所有人可见；
            - 当前无请求会话上下文 → 不做过滤（兼容遗留/测试）；
            - 否则仅 owner == 当前 session_key 时可见。
        """
        session = self._sessions.get(session_id)
        if session is None:
            return None
        watcher = current_request_session_key()
        if session.owner_session_key is not None and watcher is not None:
            if session.owner_session_key != watcher:
                return None
        return session

    def list(self, filter_owner: str | None = None) -> list[ExecSessionInfo]:
        """列出会话摘要（step84），默认按当前请求会话做 owner 隔离。

        Args:
            filter_owner: 显式指定观察者会话 key；为 None 时取
                ``current_request_session_key()``。

        Returns:
            ExecSessionInfo 列表，按会话 ID 排序。仅返回对观察者可见的会话
            （owner == 观察者，或 owner 为 None 的遗留会话）。

        隔离规则同 ``get``：会话 owner 为 None → 全员可见；观察者无上下文 → 不过滤；
        否则仅 owner == 观察者时可见。
        """
        watcher = filter_owner if filter_owner is not None else current_request_session_key()
        now = time.monotonic()
        infos = []
        for session in self._sessions.values():
            # owner 隔离：隐藏不属于当前观察者的会话（owner 为 None 视为全员可见）
            if session.owner_session_key is not None and watcher is not None:
                if session.owner_session_key != watcher:
                    continue
        ...
        infos.sort(key=lambda x: x.session_id)
        return infos
...
class ListExecSessionsTool(Tool):
    ...
    def description(self) -> str:
        ...
        return (
            ...
            "elapsed time, status, and command preview. "
