...
from step30.config.schema import Config
from step30.context import ContextBuilder
from step30.loop import AgentLoop, TurnContext
from step30.memory import MemoryStore
from step30.runtime_context import (
    RUNTIME_CONTEXT_END,
    RUNTIME_CONTEXT_TAG,
    RuntimeContextBlock,
...
class TestLoopRuntimeContext:
    ...
    @pytest.mark.asyncio
    async def test_state_build_attaches_blocks_in_memory_only(self, tmp_path):
        loop = _mk_loop(tmp_path)
        ...
        # 取舍验证：runtime 上下文只拼进内存 initial_messages，不写入会话历史。
        assert ctx.session.messages[-1]["content"] == "hello"
