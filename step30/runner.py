...
from step30.hook import AgentHook, AgentHookContext, AgentRunHookContext
from step30.llm import LLMResponse
from step30.provider import LLMProvider
from step30.tool import Tool, ToolRegistry
...
# step30（A8）：欠费/配额不足类错误（HTTP 402 或 billing 语义 token）不会被
...
class AgentRunSpec:
    ...
    Attributes:
        ...
        finalize_on_max_iterations: max_iterations 边界是否产出收尾响应。
            ...
            （step29 仅开关语义；step30 将补充"收尾 = 一次无工具 LLM 调用"。）
    """
    ...
    # step30（A8）：可定制文案——error 终止时用 ``error_message``（缺省回退
...
class AgentRunner:
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        from step30.security.workspace_access import bind_workspace_scope, reset_workspace_scope
    ...
    def _error_result(
        ...
        # step30（A8）：文案优先级——自定义 error_message > 欠费识别
    ...
    async def _request_model(
        ...
        # step30（A8）：usage 缺失时按文本长度估算（~4 字符/token），保证
    ...
    def _append_injected_messages(
        ...
        from step30.session.history_visibility import is_hidden_history_message
    ...
    async def _run_loop(
        ...
        for iteration in range(spec.max_iterations):
            ...
            # step30：对齐 nanobot——模型响应后先别关流（工具执行/重试/注入
            ...
            # step30（A8）：工具只在 ``should_execute_tools`` 下执行——
            ...
            # step30：对齐 nanobot——先查注入再决定关流：还有注入续跑时
        ...
        # step30：边界处关流——可续跑时保持流存活（resuming=True），否则收尾。
        await hook.on_stream_end(iter_ctx, resuming=not spec.finalize_on_max_iterations)
        final_content = None
        if spec.finalize_on_max_iterations:
            # step30（A8）：收尾文案可定制（缺省 _MAX_ITERATIONS_FALLBACK）。
